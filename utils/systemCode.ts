...
const getDefaultSystemCodeStartNumber = (moduleName?: string | null) => {
  ...
  return normalized === "customers"
    ? DEFAULT_CUSTOMER_SYSTEM_CODE_START_NUMBER
    : DEFAULT_SYSTEM_CODE_START_NUMBER;
};
...
const normalizeSystemCodeStartNumber = (value: unknown) => {
  ...
  if (numeric > MAX_SYSTEM_CODE_SEQUENCE_NUMBER)
    return DEFAULT_SYSTEM_CODE_START_NUMBER;
...
const normalizeSystemCodeNumberWidth = (value: unknown) => {
  ...
  return normalized > 0 && normalized <= MAX_SYSTEM_CODE_NUMBER_WIDTH
    ? normalized
    : null;
};
...
const formatSystemCodeValue = (
  prefix: string,
  numericValue: number,
  numberWidth: number | null,
) => {
  const normalizedNumber = Math.max(0, Math.trunc(numericValue));
  const suffix = numberWidth
    ? String(normalizedNumber).padStart(numberWidth, "0")
    : String(normalizedNumber);
  return `${prefix}${suffix}`;
};

const parseSystemCodeNumber = (
  systemCode: unknown,
  prefix: string,
): number | null => {
  ...
  if (!Number.isFinite(numeric) || numeric > MAX_SYSTEM_CODE_SEQUENCE_NUMBER)
    return null;
...
const scanLastSystemCodeNumber = async (
  ...
  for (;;) {
    ...
    rows.forEach((row) => {
      ...
      if (numeric !== null)
        maxExistingNumber = Math.max(maxExistingNumber, numeric);
    });
...
const normalizeLegacyCustomerDefaults = (
  config: ResolvedSystemCodeConfig,
  namingSettings: any,
  ...
  const hasExplicitWidth =
    namingSettings?.numberWidth !== undefined &&
    namingSettings?.numberWidth !== null;
  const normalizedPrefix = String(config.prefix || "")
    .trim()
    .toUpperCase();
  ...
  if (
    !hasExplicitWidth &&
    isCustomerDefaultPrefix &&
    Number(config.startNumber) === DEFAULT_SYSTEM_CODE_START_NUMBER
  ) {
    return {
      prefix: "C",
...
const loadModuleSettings = async (supabaseClient: any) => {
  ...
  if (moduleSettingsCache && moduleSettingsCacheOrgId === orgId)
    return moduleSettingsCache;
  if (moduleSettingsPromise && moduleSettingsPromiseOrgId === orgId)
...
export const resolveSystemCodeConfig = async (
  ...
  try {
    ...
    const namingSettings =
      modules?.[normalizedModule]?.general?.systemCodeNaming || {};
    ...
    return normalizeLegacyCustomerDefaults(
      resolvedConfig,
      namingSettings,
      normalizedModule,
    );
...
export const buildClientFallbackSystemCode = async (
  ...
  const { prefix, startNumber, numberWidth } = await resolveSystemCodeConfig(
    supabaseClient,
    moduleName,
  );
  ...
  try {
    ...
    const counterLastNumber = await getCounterLastNumber(
      supabaseClient,
      sourceTable,
      orgId,
      prefix,
    );
    const rpcLastNumber = await getRpcLastNumber(
      supabaseClient,
      sourceTable,
      orgId,
      prefix,
    );
    const knownLastNumbers = [
      cachedLastNumber,
      counterLastNumber,
      rpcLastNumber,
    ]
    ...
    let maxExistingNumber =
      knownLastNumbers.length > 0
        ? Math.max(...knownLastNumbers)
        : await scanLastSystemCodeNumber(
            supabaseClient,
            sourceTable,
            orgId,
            prefix,
    ...
    maxExistingNumber = Math.max(
      Number(maxExistingNumber) || 0,
      startNumber - 1,
    );
    const nextNumber = Math.min(
      maxExistingNumber + 1,
      MAX_SYSTEM_CODE_SEQUENCE_NUMBER,
    );
