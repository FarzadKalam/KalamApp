...
export const useListPrintManager = ({
  ...
  const availableTemplates = useMemo(() => {
    ...
    const baseTemplates =
      scopedTemplates.length > 0
        ? scopedTemplates
        : buildDefaultTemplatesForModule(moduleId, "list").filter(
            (tpl) => tpl.isActive !== false,
          );
    return buildPrintLetterheadVariants(
      baseTemplates,
      companyInfo?.print_letterheads || [],
    );
  }, [
    companyInfo?.print_letterheads,
    moduleId,
  ...
  const showImageDisplayModeControl = useMemo(() => {
    ...
    const hasImageField = printableFieldsForTemplate.some(
      (field) => String(field?.type || "").toLowerCase() === "image",
    );
    return (
      hasImageField ||
      templateId.includes("_catalog_") ||
      contentHtml.includes("system.list_catalog_")
    );
  }, [
          ...
          const imageKeys = printableFieldsForTemplate
            .filter(
              (field) =>
                ...
                sanitizedKeys.includes(field.key),
            )
            .map((field) => field.key)
            .slice(0, 1);
  ...
  const selectedFields = useMemo(() => {
    const hasSelectionState = Object.prototype.hasOwnProperty.call(
      selectedPrintFields,
      selectedTemplateId,
    );
    const fieldMap = new Map(
      printableFieldsForTemplate.map((field) => [
  ...
  const selectedColumns = useMemo(() => {
    ...
    const imageFields = resolved
      .filter((field) => String(field?.type || "").toLowerCase() === "image")
      .slice(0, 1);
    const contentFields = resolved
      .filter((field) => String(field?.type || "").toLowerCase() !== "image")
      .slice(0, 5);
    ...
    (
      ...
      if (path === "system.print_date")
        return toPersianNumber(
          safeJalaliFormat(printRenderTimestampRef.current, "YYYY/MM/DD HH:mm"),
        );
      ...
      if (path.startsWith("system.extra.")) {
        ...
        if (
          extraKey === "summary_html" &&
          !Object.prototype.hasOwnProperty.call(extraSystemValues, extraKey)
        ) {
          return renderedSummaryTable;
        }
  ...
  const renderTemplateSection = useCallback(
    (
      ...
      const htmlWithContext =
        renderedContextTable && !rawHtml.includes("system.list_context_table")
          ? rawHtml.replace(
    ...
    (templateId: string, fieldName: string) => {
      setSelectedPrintFields((prev) => {
        ...
        if (current.includes(fieldName)) {
          return {
            ...prev,
            [templateId]: current.filter((item) => item !== fieldName),
          };
        }
        if (String(templateId).includes("_catalog_a4_portrait")) {
          if (current.length >= 6) return prev;
          const targetField = printableFieldsForTemplate.find(
            (field) => field.key === fieldName,
          );
          const isImageField =
            String(targetField?.type || "").toLowerCase() === "image";
          if (isImageField) {
            const hasImageAlready = current.some((key) => {
              const field = printableFieldsForTemplate.find(
                (item) => item.key === key,
              );
          ...
          if (String(templateId).includes("_catalog_a4_portrait")) {
            ...
            if (isImageField) {
              const hasImageAlready = next.some((key) => {
                const matchedField = printableFieldsForTemplate.find(
                  (item) => item.key === key,
                );
                return (
                  String(matchedField?.type || "").toLowerCase() === "image"
                );
              });
  ...
  const preparePrint = useCallback(() => {
    const shouldUseNativePdfFlow = Boolean(
      buildNativeListPrintFlowRef.current(),
    );
  ...
  const renderPrintCard = useCallback(() => {
    ...
    const pageMargins = {
      ...
      right: Number(
        selectedStoredTemplate.pageMarginRight ?? PAGE_MARGINS.right,
      ),
      bottom: Number(
        selectedStoredTemplate.pageMarginBottom ?? PAGE_MARGINS.bottom,
      ),
    ...
    if (isOrgLetterheadTemplate && selectedOrgLetterhead) {
      ...
      return React.createElement(
        ...
        ...pagedRows.map((pageRows, pageIndex) => {
          ...
          const contentHtml = renderTemplateSection(
            selectedStoredTemplate.contentHtml,
            pageIndex,
            pagedRows.length,
            pageRows,
          ...
          return React.createElement(
            "div",
            {
              ...
              style: {
                ...
                overflow: "hidden",
                pageBreakAfter:
                  pageIndex < pagedRows.length - 1 ? "always" : "auto",
  ...
  const buildNativeListPrintFlow = useCallback(() => {
    ...
    if (isOrgLetterheadTemplate && selectedOrgLetterhead) {
      ...
      return buildNativeCustomPrintFlowHtml({
        ...
        pageMargins: {
          ...
          bottom:
            (metrics.heightMm *
              Math.max(0, 100 - bodyItem.y - bodyItem.height)) /
            100,
          left: (metrics.widthMm * bodyItem.x) / 100,
        },
        sectionPadding: "0",
        contentHtml: renderTemplateSection(
          selectedStoredTemplate.contentHtml,
          0,
          1,
          allRows,
    ...
    return buildNativeCustomPrintFlowHtml({
      ...
      headerHeightPx: showHeader
        ? Number(selectedStoredTemplate.headerHeight || 84)
        : 0,
      footerHeightPx: showFooter
        ? Number(selectedStoredTemplate.footerHeight || 62) +
          signatureHeightPx +
          18
        : 0,
      showHeader,
