...
این پروژه در فرانت فایل‌ها را از طریق API استاندارد `storage-api` آپلود می‌کند. یعنی فرانت مستقیم به S3 پارس‌پک وصل نمی‌شود؛ فرانت به `storage-api` خود Supabase وصل می‌شود و خود `storage-api` فایل را داخل S3-compatible storage پارس‌پک می‌نویسد.

برای سازگاری با endpoint فعلی پارس‌پک، آپلود resumable/TUS در فرانت استفاده نمی‌شود. مسیر استاندارد Storage با همین تنظیمات روی سرور تست شده و برای فایل‌های تا سقف ۵۰۰ مگابایت استفاده می‌شود.
