-- Phase 289: Process runtime visibility on every linked record
-- Runtime readers now include process links stored on task recurrence metadata,
-- so a process appears on every related module record, not only the primary one.
