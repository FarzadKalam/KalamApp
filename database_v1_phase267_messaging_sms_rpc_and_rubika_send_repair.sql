-- Repair messaging SMS report RPC stability and keep the live list fast.

begin;

create index if not exists idx_outbound_messages_org_sms_message_at_v2
...
as $$
  ...
  order by coalesce(m.received_at, m.sent_at, m.created_at) desc nulls last, m.created_at desc
