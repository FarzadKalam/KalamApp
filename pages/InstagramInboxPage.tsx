...
const InstagramInboxPage: React.FC = () => {
  ...
  return (
    <div className="flex h-full min-h-0 flex-col overflow-hidden rounded-xl border border-slate-200/70 bg-white dark:border-white/[0.08] dark:bg-[#161216]" dir="rtl">
      <div className="flex items-center justify-between gap-3 border-b border-slate-200 px-4 py-3 dark:border-white/[0.08]">
        <div className="flex items-center gap-2 font-semibold"><InstagramOutlined className="text-pink-600" /> صندوق اینستاگرام</div>
        <div className="flex items-center gap-2"><Button size="small" type={surface === 'direct' ? 'primary' : 'default'} onClick={() => setSurface('direct')}>دایرکت‌ها</Button><Button size="small" type={surface === 'comments' ? 'primary' : 'default'} onClick={() => setSurface('comments')}>کامنت‌ها</Button><Button size="small" icon={<ReloadOutlined />} onClick={() => void loadInbox()} loading={loading}>بروزرسانی</Button><Tooltip title="تنظیمات ویترین محصولات"><Button size="small" icon={<SettingOutlined />} onClick={() => setShowcaseSettingsOpen(true)}>تنظیمات</Button></Tooltip></div>
      </div>
      ...
      <div className="grid min-h-0 flex-1 grid-cols-1 lg:grid-cols-[330px_minmax(0,1fr)]">
        <div className="min-h-0 overflow-y-auto border-l border-slate-200 dark:border-white/[0.08]">
          <List loading={loading} locale={{ emptyText: 'گفتگویی برای این حساب‌ها ثبت نشده است.' }} dataSource={visibleConversations} renderItem={(item) => {
            ...
            return <List.Item className={`cursor-pointer px-3 py-3 ${item.id === activeConversationId ? 'bg-pink-50 dark:bg-pink-500/10' : ''}`} onClick={() => setActiveConversationId(item.id)}>
              <List.Item.Meta avatar={<Avatar src={item.contacts?.profile_photo_url}>{name[0]}</Avatar>} title={<div className="flex items-center justify-between gap-2"><span className="truncate">{name}</span><span className="text-[10px] text-slate-400">{formatTime(item.last_message_at)}</span></div>} description={<div><div className="truncate">{item.last_message_preview || 'بدون پیام متنی'}</div><div className="mt-1 flex gap-1">{account ? <Tag color="magenta" className="m-0">@{account.username}</Tag> : null}{(item.tags || []).slice(0, 2).map((tag) => <Tag key={tag} className="m-0">{tag}</Tag>)}</div></div>} />
            </List.Item>;
          }} />
        </div>
        <div className="flex min-h-0 flex-col">
          {selectedConversation ? <>
            <div className="flex items-center justify-between border-b border-slate-200 px-4 py-3 dark:border-white/[0.08]"><div className="flex items-center gap-2"><Avatar src={selectedConversation.contacts?.profile_photo_url}>{(selectedConversation.contacts?.display_name || selectedConversation.contacts?.username || 'ا')[0]}</Avatar><div><div className="font-medium">{selectedConversation.contacts?.display_name || selectedConversation.contacts?.username || 'کاربر اینستاگرام'}</div><div className="text-xs text-slate-400">گفتگوی اینستاگرام</div></div></div><div className="flex gap-1">{(selectedConversation.tags || []).map((tag) => <Tag key={tag}>{tag}</Tag>)}</div></div>
            <div className="min-h-0 flex-1 space-y-3 overflow-y-auto bg-slate-50/60 p-4 dark:bg-black/10">{messages.length ? messages.map((item) => <div key={item.id} className={`flex ${item.direction === 'outbound' ? 'justify-start' : 'justify-end'}`}><div className={`max-w-[78%] rounded-2xl px-3 py-2 text-sm ${item.direction === 'outbound' ? 'bg-pink-600 text-white' : 'bg-white shadow-sm dark:bg-white/[0.08]'}`}><div className="whitespace-pre-wrap">{item.content_text || 'پیام غیرمتنی'}</div><div className="mt-1 text-[10px] opacity-70">{formatTime(item.created_at)}</div></div></div>) : <Empty description="پیامی برای این گفتگو نیست." />}</div>
            <div className="border-t border-slate-200 px-3 py-2 dark:border-white/[0.08]">
