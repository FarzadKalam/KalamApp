...
import {
  buildCopyPayload,
  copyProcessTemplateStagesRelations,
  copyProductionOrderRelations,
  copyWebFormFieldsRelations,
  detectCopyNameField,
} from "../utils/recordCopy";
import { attachTaskCompletionIfNeeded } from "../utils/taskCompletion";
import {
  getTaskRelationFieldKey,
...
import {
  buildRecordReferenceKey,
  fetchRecordReferenceLabels,
} from "../utils/recordReference";
import {
...
const MergeRecordsModal = React.lazy(
  () => import("../components/moduleList/MergeRecordsModal"),
);
const GridView = React.lazy(() => import("../components/moduleList/GridView"));
const ModuleCalendarView = React.lazy(
  () => import("../components/moduleList/CalendarView"),
);
const ReservationAvailabilityPanel = React.lazy(
  () => import("../components/reservations/ReservationAvailabilityPanel"),
);
const RenderCardItem = React.lazy(
  () => import("../components/moduleList/RenderCardItem"),
);
const BulkProductsCreateModal = React.lazy(
  () => import("../components/products/BulkProductsCreateModal"),
);
const WorkflowsManager = React.lazy(
  () => import("../components/workflows/WorkflowsManager"),
);
const GoalsManager = React.lazy(
  () => import("../components/goals/GoalsManager"),
);
const GoalProgressSlider = React.lazy(
  () => import("../components/goals/GoalProgressSlider"),
);
const ExcelImportWizard = React.lazy(
  () => import("../components/moduleList/ExcelImportWizard"),
);
const ListPrintRuntime = React.lazy(
  () => import("../components/moduleList/ListPrintRuntime"),
);
const MessageComposerModal = React.lazy(
  () => import("../components/MessageComposerModal"),
);
...
const DeleteModuleRecordsModal = React.lazy(
  () => import("../components/moduleDelete/DeleteModuleRecordsModal"),
);
const OnlineCatalogManagerModal = React.lazy(
  () => import("../components/onlineCatalog/OnlineCatalogManagerModal"),
);
const BillboardStatusChangeModal = React.lazy(
  () => import("../components/billboards/BillboardStatusChangeModal"),
);
const RetailInvoiceSettingsModal = React.lazy(
  () => import("../components/invoices/RetailInvoiceSettingsModal"),
);

const DEFAULT_LIST_PAGE_SIZE = 20;
const SELECTED_RECORD_FETCH_CHUNK_SIZE = 25;
const TAG_VIEW_FILTER_FIELD = "__tag_view_filter__";
const MODULE_LIST_LIVE_WATERMARK_STORAGE_PREFIX =
  "kalam:module-list-live-watermark";

const getModuleListLiveMarkerStamp = (
  ...
  const updatedAt = Date.parse(
    String(marker.updated_at || marker.received_at || ""),
  );
...
const writeModuleListLiveWatermark = (
  key?: string | null,
  value?: number | null,
) => {
...
const getTagViewFilterMeta = (
  filter: any,
  moduleConfig?: ModuleDefinition | null,
) => {
  if (!filter || typeof filter !== "object") return null;

  const tagFields = (moduleConfig?.fields || []).filter(
    (field) => field.type === FieldType.TAGS,
  );
  if (tagFields.length === 0) return null;

  const tagFieldKeys = new Set(
    tagFields.map((field) => String(field.key || "").trim()).filter(Boolean),
  );
  ...
  const isCustomTagFilter =
    filter._isTagViewFilter === true || directField === TAG_VIEW_FILTER_FIELD;
  const tagFieldKey =
  ...
  const rawTagIds = Array.isArray(payload?.tagIds)
    ? payload.tagIds
  ...
  const tagIds = (
    Array.isArray(rawTagIds)
      ? rawTagIds
      : rawTagIds !== undefined && rawTagIds !== null && rawTagIds !== ""
        ? [rawTagIds]
        : []
  )
  ...
  return {
    ...
    sourceOperator: String(
      payload?.sourceOperator ||
        filter._displayOperator ||
        filter.operator ||
        "eq",
    ).trim(),
...
const sanitizeModuleVisibleColumns = (
  ...
  const sanitized = (Array.isArray(columns) ? columns : [])
    ...
    .filter((key) => {
      if (
        ...
        CASH_BANK_LEGACY_ACCOUNT_KEYS.has(key)
      )
        return false;
      if (moduleId === "attendance_logs" && key === "closure_status")
        return false;
  ...
  if (
    moduleId === "cash_bank_operations" &&
    allowedFieldKeys.has("image_url")
  ) {
...
const normalizeVisibleColumnsForView = (
  ...
  const sanitized = sanitizeModuleVisibleColumns(
    moduleId,
    moduleConfig,
    columns,
  );
  const builtInViewKey = String(
    (currentView?.config as any)?.__built_in_view_key || "",
  ).trim();
  const isDefaultView =
    !currentView ||
  ...
  if (
    ...
    sanitized.length === allFieldKeys.length
  ) {
    return [];
  }
  return sanitized;
...
const buildModuleListRowSelect = (
  ...
  const extraSelectKeys = new Set(
    MODULE_LIST_EXTRA_SELECT_KEYS[moduleConfig.id] || [],
  );
  ...
  const MANAGED_SYSTEM_COLUMNS = new Set([
    "assignee_type",
    "assignee_id",
    "assignee_role_id",
  ]);
  ...
  const addKnownKey = (key?: string | null, force = false) => {
    ...
    if (
      !force &&
      shouldDeferHeavyFields &&
      shouldDeferModuleListField(moduleConfig.id, field as any)
    )
      return;
    if (
      normalized === "id" ||
      moduleFieldKeys.has(normalized) ||
      selectableFieldKeys.has(normalized) ||
      extraSelectKeys.has(normalized) ||
      (MANAGED_SYSTEM_COLUMNS.has(normalized) && moduleSupportsAssignee)
    ) {
      addKey(normalized);
    }
  };
  ...
  getModuleListVisibleFields(moduleConfig, visibleColumns || undefined).forEach(
    (field) => addKnownKey(field.key),
  );
  collectCrudFilterFields(options?.filters).forEach((key) => addKnownKey(key));
  (options?.sorters || []).forEach((sorter) =>
    addKnownKey(String(sorter?.field || "")),
  );
  ...
  (moduleConfig.fields || []).forEach((field) => {
    ...
    if (field.type === FieldType.IMAGE || field.type === FieldType.LOCATION)
      addKnownKey(key);
    if (
  ...
  if (options?.viewMode === ViewMode.CALENDAR)
    addKnownKey(options.calendarDateField);
  if (options?.viewMode === ViewMode.MAP) {
    (moduleConfig.fields || []).forEach((field) => {
      if (field.type === FieldType.LOCATION || field.key === "location")
        addKnownKey(field.key);
    });
  }
...
const readPersistedModuleListState = (
  moduleId?: string | null,
  suffix?: string | null,
): PersistedModuleListState | null => {
  if (typeof window === "undefined" || !moduleId) return null;
  try {
    const raw = window.localStorage.getItem(
      buildModuleListStateKey(moduleId, suffix),
    );
    if (!raw) return null;
...
const MODULE_LIST_CREATED_AT_DEFAULT_SORT_MODULES = new Set([
  "automation_execution_reports",
]);
const MODULE_LIST_PLANNED_COUNT_MODULE_IDS = new Set([
  "automation_execution_reports",
]);
...
const getDefaultSorters = (
  ...
  const configuredSorters = normalizeCrudSorters(
    moduleConfig.defaultSorters as CrudSort[] | undefined,
  );
  if (configuredSorters.length > 0) return configuredSorters;
  const moduleId = String(moduleConfig.id || "");
  const hasCreatedAt = moduleConfig.fields?.some(
...
const stripStableIdSorter = (sorters?: CrudSort[] | null): CrudSort[] => {
  ...
  if (
    normalized.length > 1 &&
    String(normalized[normalized.length - 1]?.field || "") === "id"
  ) {
...
const sanitizeSorters = (
  ...
  const sanitized = sorters.filter((sorter) =>
    sortableFields.has(String(sorter.field || "").trim()),
  );
  return sanitized.length > 0 ? sanitized : getDefaultSorters(moduleConfig);
};

const sanitizePersistedSorters = (
  sorters?: CrudSort[] | null,
  moduleConfig?: ModuleDefinition | null,
): CrudSort[] => {
...
const resolveCrudSortersWithDefault = (
  sorters?: CrudSort[] | null,
  moduleConfig?: ModuleDefinition | null,
): CrudSort[] => {
...
const areCrudSortersEqual = (
  left?: CrudSort[] | null,
  right?: CrudSort[] | null,
) =>
  JSON.stringify(normalizeCrudSorters(left)) ===
  JSON.stringify(normalizeCrudSorters(right));
const toHeaderOnlyModule = (
...
const getBulkBuildSourceModule = (
  moduleId?: string | null,
): BulkBuildSourceModule | null => {
...
const getCatalogRecordBuildMeta = (
  record: any,
  sourceModule: BulkBuildSourceModule,
) => {
  ...
  const buyPrice = isBillboard
    ? unitPrice
    : Number(record?.buy_price || 0) || 0 || unitPrice;
  const profitPercentage =
    buyPrice > 0 ? roundMoney(((unitPrice - buyPrice) / buyPrice) * 100) : 0;
...
const buildPackageItemsFromRecords = (
  records: any[],
  sourceModule: BulkBuildSourceModule,
) =>
  records
...
const buildPriceListItemsFromRecords = (
  records: any[],
  sourceModule: BulkBuildSourceModule,
) => {
...
const ModuleListContentSkeleton: React.FC<{ viewMode: ViewMode }> = ({
  viewMode,
}) => {
...
export const ModuleListRefine: React.FC<{
  ...
  const [surveyTemplateSnapshot, setSurveyTemplateSnapshot] = useState(() =>
    normalizeSurveyTemplateSnapshot({}),
  );
  const baseModuleConfig = resolvedModuleId ? MODULES[resolvedModuleId] : null;
  const { moduleConfig: planBaseModuleConfig } =
    useContentCalendarPlanModule(baseModuleConfig);
  const { enabled: retailSalesInvoiceEnabled } = useRetailSalesInvoiceFeature();
  const moduleConfig = useMemo(
    () =>
      planBaseModuleConfig &&
      supportsWebFormTemplateRuntime(planBaseModuleConfig)
        ? buildSurveyRuntimeModule(
            planBaseModuleConfig,
            surveyTemplateSnapshot,
            "list",
          )
        : planBaseModuleConfig,
  ...
  const searchTargetField = useMemo(() => {
    ...
    const priorityField = moduleConfig.fields.find((f) =>
      priorityKeys.includes(f.key),
    );
    if (priorityField) return priorityField.key;
    const textField = moduleConfig.fields.find(
  ...
  >({});
  const [fieldPermissionsModuleId, setFieldPermissionsModuleId] = useState<
    string | null
  >(null);
  const fieldPermissionsReady =
    fieldPermissionsModuleId === String(resolvedModuleId || "");
  ...
  const effectiveInitialViewFilters = useMemo(
    () =>
      Array.isArray(initialViewFiltersOverride) &&
      initialViewFiltersOverride.length > 0
  ...
  const [listVisibleRowKeys, setListVisibleRowKeys] = useState<
    React.Key[] | null
  >(null);
  ...
  const [viewFiltersState, setViewFiltersState] = useState<CrudFilters>(
    effectiveInitialViewFilters,
  );
  ...
  const [loadedTagsRecordIdsSignature, setLoadedTagsRecordIdsSignature] =
    useState("");
  ...
  const [allUsers, setAllUsers] = useState<any[]>(
    () => cachedOptionSnapshot?.allUsers || [],
  );
  const [allRoles, setAllRoles] = useState<any[]>(
    () => cachedOptionSnapshot?.allRoles || [],
  );
  ...
  const [permissionViewConditions, setPermissionViewConditions] =
    useState<ViewConditionGroup | null>(null);
  const [permissionConditionAllowedIds, setPermissionConditionAllowedIds] =
    useState<Set<string> | null>(null);
  const [permissionConditionLoading, setPermissionConditionLoading] =
    useState(false);
  const [clientPermissionPageRows, setClientPermissionPageRows] = useState<
    any[]
  >([]);
  const [clientPermissionTotal, setClientPermissionTotal] = useState(0);
  const [
    clientPermissionPaginationActive,
    setClientPermissionPaginationActive,
  ] = useState(false);
  ...
  const [currentPermissionMap, setCurrentPermissionMap] =
    useState<PermissionMap | null>(null);
  const [currentSoftwareRole, setCurrentSoftwareRole] = useState<string | null>(
    null,
  );
  const [recordLockMap, setRecordLockMap] = useState<
    Map<string, RecordLockState>
  >(() => new Map());
  ...
  const [isRetailInvoiceSettingsOpen, setIsRetailInvoiceSettingsOpen] =
    useState(false);
  ...
  const [bulkSmsSourceRecord, setBulkSmsSourceRecord] = useState<Record<
    string,
    any
  > | null>(null);
  ...
  const [isOnlineCatalogManagerOpen, setIsOnlineCatalogManagerOpen] =
    useState(false);
  const [isBillboardStatusChangeOpen, setIsBillboardStatusChangeOpen] =
    useState(false);
  ...
  const surveyTemplateScopedId = useMemo(
    () =>
      baseModuleConfig && supportsWebFormTemplateRuntime(baseModuleConfig)
        ? getSurveyTemplateScopedIdFromCrudFilters(
            buildMergedFilters(viewFiltersState, searchTerm, columnFilters),
          )
        : null,
  ...
  useEffect(() => {
    ...
    if (
      !baseModuleConfig ||
      !supportsWebFormTemplateRuntime(baseModuleConfig) ||
      !surveyTemplateScopedId
    ) {
    ...
    const run = async () => {
      try {
        const definition = await loadSurveyTemplateDefinition(
          supabase,
          surveyTemplateScopedId,
        );
        if (cancelled) return;
        setSurveyTemplateSnapshot(
          normalizeSurveyTemplateSnapshot(definition?.snapshot || {}),
        );
      } catch {
        if (!cancelled) {
          setSurveyTemplateSnapshot(normalizeSurveyTemplateSnapshot({}));
  ...
  const moduleListLivePendingMarkerRef =
    useRef<ModuleListLocalInvalidationMarker | null>(null);
  const [hasListInitialPaintCompleted, setHasListInitialPaintCompleted] =
    useState(false);
  const [deferredModuleRowsById, setDeferredModuleRowsById] = useState<
    Record<string, any>
  >({});
  const [utilitySlotHeight, setUtilitySlotHeight] = useState<number | null>(
    null,
  );
  ...
  const [isMobileViewModeSheetOpen, setIsMobileViewModeSheetOpen] =
    useState(false);
  ...
  const {
    ...
    meta: {
      ...
      count: MODULE_LIST_PLANNED_COUNT_MODULE_IDS.has(
    ...
    filters: {
      permanent: (moduleConfig?.permanentFilters ?? []) as any,
      initial: effectiveInitialFilters,
    },
  ...
  const stableSorters = useMemo(
    () =>
      ensureStableCrudSorters(
        resolveCrudSortersWithDefault(sorters as CrudSort[], moduleConfig),
      ),
  ...
  const effectiveBaseAllData = useMemo(() => {
    ...
    const fallbackRows = cashBankFallbackRows.filter(
      (row: any) => !existingIds.has(String(row?.id || "").trim()),
    );
  ...
  const deferredModuleRowsSignature = useMemo(
    () =>
      (effectiveBaseAllData || [])
        .map(
          (row: any) =>
            `${String(row?.id || "").trim()}:${String(row?.updated_at || "").trim()}`,
        )
  ...
  useEffect(() => {
    if (
      ...
      setDeferredModuleRowsById((current) =>
    ...
    currentOrgId,
    dataResource,
    deferredModuleRowsSignature,
    effectiveBaseAllData,
    moduleListDeferredFieldKeys,
    viewMode,
  ]);
  ...
  const effectiveAllData = useMemo(
    () =>
      (effectiveBaseAllData || []).map((row: any) => {
        const deferredRow =
          deferredModuleRowsById[String(row?.id || "").trim()];
  ...
  const isRecordIdLocked = useCallback(
    (recordId?: string | null, fallbackRecord?: any) => {
      const normalizedId = String(recordId || fallbackRecord?.id || "").trim();
      if (!normalizedId) return false;
      return (
        recordLockMap.has(normalizedId) ||
        getRecordLockStateFromRecord(fallbackRecord).isLocked
      );
    },
    [recordLockMap],
  );
  const hasLockedSelectedRows = useMemo(
    () =>
      selectedRowKeys.some((key) =>
        isRecordIdLocked(String(key), selectedRowsMap[String(key)]),
      ),
  ...
  const canBulkSendSms = useMemo(
    () =>
      ["customers", "suppliers", "employees"].includes(
        String(resolvedModuleId || ""),
      ),
    [resolvedModuleId],
  );
  ...
  const isListPageSizeReady =
    viewMode !== ViewMode.LIST || Number(pageSize || 0) > 0;
  ...
  const fetchRowsByIdsPreservingOrder = useCallback(
    async (ids: string[], serverFilters: CrudFilters) => {
  ...
  const fetchAllRowsForFilters = useCallback(
    async (serverFilters: CrudFilters) => {
      if (!resolvedModuleId) {
        return { rows: [], total: 0 };
      }
      const resource = dataResource || resolvedModuleId;
      ...
      while (currentPage <= totalPages) {
        ...
        if (currentPage === 1) {
          ...
          pageRows.length < SCAN_PAGE_SIZE &&
          totalPages === Number.MAX_SAFE_INTEGER
        ) {
          break;
        }
  ...
  const fetchOrderedIdsForFilters = useCallback(
    async (serverFilters: CrudFilters) => {
  ...
  const resolveOrderedTagFilteredIds = useCallback(
    async (serverFilters: CrudFilters) => {
      if (!resolvedModuleId || !dataResource) return [];
      ...
      const hasNegativeTagFilter = normalizedActiveTagViewFilters.some(
        (item) => {
          const operator = String(item?.sourceOperator || "eq").trim();
          return (
            operator === "neq" ||
      ...
      const unionTagIds = Array.from(
        new Set(
          normalizedActiveTagViewFilters
      ...
      const selectedTagsByRecord = new Map<string, Set<string>>(
        Object.entries(selectedTagIdMap).map(([recordId, tagIds]) => [
          recordId,
          new Set(tagIds),
        ]),
      );
      const allTagsByRecord = new Map<string, Set<string>>(
        Object.entries(allTagIdMap).map(([recordId, tagIds]) => [
          recordId,
          new Set(tagIds),
        ]),
      );
      ...
      normalizedActiveTagViewFilters.forEach((tagFilter) => {
        ...
        if (operator === "multi_count_gt" || operator === "multi_count_lt") {
          ...
          (taggedRecordIds || new Set<string>()).forEach((recordId) => {
            const count = (allTagsByRecord.get(recordId) || new Set<string>())
              .size;
            if (
              (operator === "multi_count_gt" && count > selectedCount) ||
              (operator === "multi_count_lt" && count < selectedCount)
            ) {
        ...
        if (operator === "is_null") {
          (taggedRecordIds || new Set<string>()).forEach((recordId) =>
            excludedRecordIds.add(recordId),
          );
          return;
        }
        ...
        if (
          ...
          matchedRecordIds.forEach((recordId) =>
            excludedRecordIds.add(recordId),
          );
        }
      });
      ...
      const includedRecordIds =
        positiveMatchSets.length > 0
          ? positiveMatchSets.reduce(
              (carry, currentSet) => {
      ...
      const canUseDirectIdFastPath =
        (!includedRecordIds ||
          includedRecordIds.size <= TAG_FAST_FILTER_LIMIT) &&
        (!excludedRecordIds.size ||
          excludedRecordIds.size <= TAG_FAST_FILTER_LIMIT) &&
        (includedRecordIds !== null ||
          excludedRecordIds.size > 0 ||
          needsAllTaggedUniverse ||
      ...
      if (canUseDirectIdFastPath) {
        ...
        const orderedFilteredIds =
          await fetchOrderedIdsForFilters(narrowedFilters);
        tagViewFilterIdsCacheRef.current = {
          signature: cacheSignature,
      ...
      const tagsByRecord = needsAllTaggedUniverse
        ? allTagsByRecord
        : selectedTagsByRecord;
      ...
      const passesTagFilters = (recordId: string) => {
        ...
        for (const tagFilter of normalizedActiveTagViewFilters) {
          ...
          if (op === "multi_count_gt" || op === "multi_count_lt") {
            ...
            if (!Number.isFinite(selectedCount) || selectedCount < 0)
              return false;
            if (op === "multi_count_gt" && !(recordTagIds.size > selectedCount))
              return false;
            if (op === "multi_count_lt" && !(recordTagIds.size < selectedCount))
              return false;
            continue;
          }
          ...
          const hasSelectedTag = tagIds.some((tagId) =>
            recordTagIds.has(tagId),
          );
      ...
      const orderedFilteredIds = baseOrderedIds.filter((id) =>
        passesTagFilters(id),
      );
  ...
  useEffect(() => {
    if (
      resolvedModuleId !== "cash_bank_operations" ||
      queryPending ||
      cashBankBackfillAttemptedRef.current
    )
      return;
  ...
  useEffect(() => {
    ...
    const restoredState = readPersistedModuleListState(
      resolvedModuleId,
      storageKeySuffix,
    );
    const restoredSorters = resolveCrudSortersWithDefault(
      restoredState?.sorters,
      moduleConfig,
    );
    ...
    setViewMode(
      restoredState?.viewMode || moduleConfig?.defaultViewMode || ViewMode.LIST,
    );
    setCurrentView(
      Array.isArray(initialViewFiltersOverride) &&
        initialViewFiltersOverride.length > 0
    ...
    try {
      ...
      if (isSaasAdminModuleId(resolvedModuleId)) {
        ...
        const canViewSaas =
          saasPerms.view === true ||
          saasPerms.edit === true ||
          (editFieldKey ? saasFields[editFieldKey] === true : false);
        const canEditSaas =
          canViewSaas &&
          (saasPerms.edit === true ||
            (editFieldKey ? saasFields[editFieldKey] === true : false) ||
            resolvedModuleId === "saas_users");
        const canDeleteSaas =
          resolvedModuleId === "saas_user_announcements" ? canEditSaas : false;
        setModulePermissions({
          view: canViewSaas,
          edit: canEditSaas,
      ...
      const nextRecordScope =
        modulePerms.record_scope ??
        (modulePerms.view === false ? "own" : "all");
      ...
      const normalizedViewConditions = normalizeViewConditionGroup(
        modulePerms.view_conditions,
      );
      const workflowPerms = permissions?.[WORKFLOWS_PERMISSION_KEY] || {};
      const goalPerms = permissions?.[GOALS_PERMISSION_KEY] || {};
      const moduleGoalAccess = resolveModuleGoalAccessPermissions(
        permissions,
        resolvedModuleId,
      );
      setModulePermissions({
      ...
      setFieldPermissionsModuleId(String(resolvedModuleId || ""));
      setPermissionViewConditions(
        hasViewConditionGroupConditions(normalizedViewConditions)
          ? normalizedViewConditions
          : null,
      );
      // build permission-level filters from view_conditions
      if (hasViewConditionGroupConditions(normalizedViewConditions)) {
        buildPermissionViewCrudFilters(normalizedViewConditions)
          .then((pFilters) => {
      ...
      setCanOpenWorkflows(
        workflowPerms.view !== false &&
          workflowPerms?.fields?.module_list_button !== false,
      );
      setCanOpenGoals(
        goalPerms.view !== false &&
          goalPerms?.fields?.module_list_button !== false &&
          moduleGoalAccess.canViewGoal,
      );
      setCanShowGoalCards(
        goalPerms.view !== false &&
          goalPerms?.fields?.module_list_cards !== false &&
          moduleGoalAccess.canViewModuleCards,
      );
    } catch (err: any) {
  ...
  const moduleListLiveWatermarkKey = useMemo(
    () =>
      buildModuleListLiveWatermarkKey(
        currentOrgId,
        resolvedModuleId,
        viewMode,
        storageKeySuffix,
  ...
  const writeModuleListLiveObservedAt = useCallback(
    (stamp?: number | null) => {
      ...
      writeModuleListLiveWatermark(
        moduleListLiveWatermarkKey,
        Math.max(previous, nextStamp),
      );
    },
    [moduleListLiveWatermarkKey],
  );

  const refreshModuleListFromLiveInvalidation = useCallback(
    async (marker?: ModuleListLocalInvalidationMarker | null) => {
      if (moduleListLiveRefetchingRef.current) {
        moduleListLivePendingMarkerRef.current =
          marker || moduleListLivePendingMarkerRef.current;
        return;
      }
      ...
      try {
        ...
        if (
          pendingMarker &&
          getModuleListLiveMarkerStamp(pendingMarker) >
            readModuleListLiveWatermark(moduleListLiveWatermarkKey)
        ) {
    ...
    [
      moduleListLiveWatermarkKey,
      tableQueryResult,
      writeModuleListLiveObservedAt,
    ],
  );

  const scheduleModuleListLiveRefresh = useCallback(
    (marker?: ModuleListLocalInvalidationMarker | null) => {
      if (typeof window === "undefined") {
  ...
  const refreshModuleListIfLocalMarkerIsStale = useCallback(() => {
    if (
      ...
      return;
    const marker = readModuleListInvalidationMarker({
      orgId: currentOrgId,
      moduleId: resolvedModuleId,
    });
    const markerStamp = getModuleListLiveMarkerStamp(marker);
    if (!markerStamp) return;
    const observedStamp = readModuleListLiveWatermark(
      moduleListLiveWatermarkKey,
    );
  ...
  const recordScope =
    modulePermissions.record_scope ??
    (modulePermissions.view === false ? "own" : "all");
  const canViewModule =
    modulePermissions.view !== false || recordScope !== "all";
  const canEditModule = modulePermissions.edit !== false;
  const canDeleteModule =
    modulePermissions.delete !== false && moduleConfig?.disableDelete !== true;
  const canLockRecords = canUseRecordLockPermission(
    currentPermissionMap,
    resolvedModuleId,
    "lock",
    currentSoftwareRole,
  );
  const canUnlockRecords = canUseRecordLockPermission(
    currentPermissionMap,
    resolvedModuleId,
    "unlock",
    currentSoftwareRole,
  );
  const canOpenModuleSettings =
    modulePermissions.view !== false &&
    fieldPermissions.__module_settings !== false;
  const isSystemManagedModule = moduleConfig?.systemManaged === true;
  const allowSystemManagedDelete =
    resolvedModuleId === "saas_user_announcements";
  const allowSystemManagedFullBulkEdit =
    resolvedModuleId === "saas_user_announcements";
  ...
  }, [canCreateModule, navigate, resolvedModuleId]);
  ...
  const shouldEvaluatePermissionConditionsOnClient = useMemo(() => {
    ...
    return conditions.some(
      (condition) => !canBuildViewConditionAsCrudFilter(condition),
    );
  }, [
    hasPermissionViewConditions,
    moduleConfig,
    normalizedPermissionViewConditions,
  ...
  useEffect(() => {
    if (
      ...
      setClientPermissionPageRows((previous) =>
    ...
    const evaluateRows = async () => {
      try {
        ...
        const rows = hasActiveTagViewFilters
          ? await (async () => {
              const orderedIds =
                await resolveOrderedTagFilteredIds(serverFilters);
          ...
          rows.map(async (record: any) => {
            ...
            try {
              const passed = await evaluateWorkflowConditions({
                conditionsAll:
                  clientPermissionViewConditions.conditions_all || [],
                conditionsAny:
                  clientPermissionViewConditions.conditions_any || [],
        ...
        const filteredRows = rows.filter((record: any) =>
          allowedRecordIds.has(String(record?.id || "").trim()),
        );
        const safePageSize = Math.max(
          1,
          Number(pageSize || DEFAULT_LIST_PAGE_SIZE),
        );
        const maxPage = Math.max(
          1,
          Math.ceil(filteredRows.length / safePageSize),
        );
        ...
        setPermissionConditionAllowedIds(
          new Set(
            filteredRows
              .slice(fromIndex, fromIndex + safePageSize)
  ...
  const imageField = moduleConfig?.fields.find(
    (f) => f.type === FieldType.IMAGE,
  )?.key;
  const tagsField = moduleConfig?.fields.find(
    (f) => f.type === FieldType.TAGS,
  )?.key;
  const tagOnlyBulkEditModule = useMemo<ModuleDefinition | null>(() => {
    if (!moduleConfig) return null;
    if (!isSystemManagedModule || !tagsField || allowSystemManagedFullBulkEdit)
      return moduleConfig;
    const tagField = moduleConfig.fields.find(
      (field) => field.type === FieldType.TAGS,
    );
    ...
    allowSystemManagedFullBulkEdit,
    isSystemManagedModule,
    moduleConfig,
    tagsField,
  ]);
  const statusField = moduleConfig?.fields.find(
    (f) => f.type === FieldType.STATUS,
  )?.key;
  const categoryField =
    resolvedModuleId === "tasks"
      ? "related_to_module"
      : moduleConfig?.fields.find(
          (f) =>
            f.key === "category" ||
            f.key === "product_category" ||
            f.key === "business_name",
        )?.key;
  const visibleListFieldKeys = useMemo(
    () =>
      getModuleListVisibleFields(moduleConfig, visibleColumns)
        .map((field) => String(field?.key || "").trim())
        .filter(Boolean),
  ...
  const shouldLoadTags = useMemo(() => {
    ...
    if (
      MODULE_LIST_TAGS_UNSUPPORTED_MODULE_IDS.has(
        String(resolvedModuleId || ""),
      )
    )
      return false;
      ...
      .filter((record: any) => {
        const normalizedRecord =
          resolvedModuleId === "cash_bank_operations"
            ...
            : record;
        return canAccessAssignedRecord(
          normalizedRecord,
          currentUserId,
          currentUserRoleId,
          recordScope,
  ...
  const normalizedAccessibleData = useMemo(() => {
    if (resolvedModuleId === "attendance_logs")
      return enrichAttendancePresenceRows(accessibleData);
    if (resolvedModuleId !== "cash_bank_operations") return accessibleData;
    return accessibleData.map((record: any) =>
      normalizeModuleFormValues(resolvedModuleId, record),
    );
  ...
  const enrichedData = useMemo(() => {
    const lockMergedData = normalizedAccessibleData.map((record: any) => {
      ...
      return lockState
        ? mergeRecordLockIntoRecord(record, lockState)
        : mergeRecordLockIntoRecord(record, null);
    });
    ...
    return lockMergedData.map((record) => ({
      ...record,
      [tf]:
        tagsMap[record.id as string] ||
        (Array.isArray(record?.[tf]) ? record[tf] : []),
    }));
  ...
  const visibleRelationFields = useMemo(
    () =>
      getModuleListVisibleFields(moduleConfig, visibleColumns).filter(
        (field: any) =>
  ...
  useEffect(() => {
    ...
    window.dispatchEvent(
      new CustomEvent(AI_CONTEXT_EVENT, {
        detail: {
          mode: "list",
          moduleId: resolvedModuleId,
          recordId: null,
          visibleRecordIds,
          selectedRecordIds: selectedRowKeys
            .map((key) => String(key || "").trim())
            .filter(Boolean),
  ...
  const openListAiDrawer = useCallback(
    (question?: string) => {
      ...
      if (typeof window !== "undefined")
        window.setTimeout(dispatchListAiContext, 0);
    },
    [dispatchListAiContext],
  );
  ...
  const taskRelationLabelRequestsSignature = useMemo(
    () =>
      taskRelationLabelRequests
        .map(
          (request) =>
            `${request.fieldKey}:${request.moduleId}:${request.recordId}`,
        )
        .join("|"),
    [taskRelationLabelRequests],
  );
  ...
  const effectiveRelationOptions = useMemo(() => {
    ...
    Object.entries(taskRelationOptionsByField).forEach(
      ([fieldKey, options]) => {
  ...
  useEffect(() => {
    ...
    setSelectedRowsMap((prev) => {
      ...
      Object.keys(nextMap).forEach((key) => {
        if (
          !selectedRowKeys.some((selectedKey) => String(selectedKey) === key)
        ) {
          delete nextMap[key];
          changed = true;
        }
  ...
  useEffect(() => {
    if (resolvedModuleId !== "tasks" || !taskRelationLabelRequests.length) {
      setTaskRelationOptionsByField((prev) =>
    ...
    const loadTaskRelationLabels = async () => {
      ...
      try {
        ...
        console.warn(
          "Could not load task relation labels for module list",
          error,
        );
      }
      ...
      taskRelationLabelRequests.forEach((request) => {
        const label = String(
          labelMap[
            buildRecordReferenceKey(request.moduleId, request.recordId)
          ] || "",
        ).trim();
        if (!label) return;
        ...
        if (
          !current.some(
            (item: any) => String(item?.value || "") === request.recordId,
          )
        ) {
          current.push({
            label,
    ...
    resolvedModuleId,
    taskRelationLabelRequests,
    taskRelationLabelRequestsSignature,
  ]);
  ...
  useEffect(() => {
    if (
      ...
      normalizedAccessibleData.length === 0
    ) {
      setRelationLabelsLoading(false);
      return;
    }
    ...
    const hydrateVisibleRelationLabels = async () => {
      try {
        ...
        const snapshot = writeModuleOptionSnapshot(resolvedModuleId, {
          relationOptions: mergeOptionMaps(
            readModuleOptionSnapshot(resolvedModuleId)?.relationOptions,
            hydratedOptions,
          ),
          allUsers,
          allRoles,
        ...
        console.warn(
          "Could not hydrate visible relation labels for module list",
          error,
        );
      } finally {
    ...
    normalizedAccessibleData,
    resolvedModuleId,
    visibleRelationFields,
  ]);
  const showListSkeleton =
    viewMode === ViewMode.LIST &&
    (!isListPageSizeReady || (!hasListInitialPaintCompleted && queryPending));
  ...
  const handleRowSelectionChange = useCallback(
    (nextKeys: React.Key[], nextRows?: any[]) => {
  ...
  useEffect(() => {
    ...
    const applySnapshotToState = (
      snapshot: ReturnType<typeof readModuleOptionSnapshot>,
    ) => {
    ...
    const mergeAndApplySnapshot = (
      patch: Parameters<typeof writeModuleOptionSnapshot>[1],
    ) => {
      if (!isActive) return;
      const current = readModuleOptionSnapshot(resolvedModuleId);
      const snapshot = writeModuleOptionSnapshot(resolvedModuleId, {
        dynamicOptions: mergeOptionMaps(
          current?.dynamicOptions,
          patch.dynamicOptions,
        ),
        relationOptions: mergeOptionMaps(
          current?.relationOptions,
          patch.relationOptions,
        ),
    ...
    const fetchOptions = async () => {
      ...
      const dynamicTask = (
        optionPlan.immediateDynamicCategories.length > 0
          ? fetchDynamicOptionsMap(
              supabase,
              optionPlan.immediateDynamicCategories,
            )
      ...
      const directoryTask = fetchAssigneeDirectory(supabase)
        .then((directory) => {
          if (!isActive) return;
          ...
          const assigneeOptions = [
            ...
            ...roleOptions.filter(
              (role) =>
                !profileOptions.some(
                  (user) => String(user.value) === String(role.value),
                ),
            ),
          ];
          ...
          (optionPlan.immediateRelationFields || []).forEach((field: any) => {
            ...
            if (
              field.type === FieldType.USER ||
              relationConfig?.targetModule === "profiles"
            ) {
              directoryOverrides[key] = profileOptions;
            } else if (
              relationConfig?.targetModule === "org_roles" ||
              relationConfig?.targetModule === "roles"
            ) {
  ...
  useEffect(() => {
    ...
    const fetchTags = async () => {
      try {
        setTagsLoading(true);
        const nextTagsMap = await fetchRecordTagsMap(
          supabase,
          resolvedModuleId,
          accessibleRecordIds,
        );
  ...
  const availableGroupFields = useMemo(() => {
    return (
      moduleConfig?.fields.filter(
        (f) =>
          (f.type === FieldType.STATUS || f.type === FieldType.SELECT) &&
          f.options &&
          f.options.length > 0,
      ) || []
    );
  ...
  const mobileViewModeOptions = useMemo(
    () => [
      ...
      ...(mapEnabled
        ...
        : []),
      ...(availableCalendarFields.length > 0
        ? [
            {
              label: "تقویم",
    ...
    const bubbles = viewFiltersState
      .map((filter, index) => {
        ...
        const fieldKey = String(
          simpleFilter?._displayField || simpleFilter?.field || "",
        );
                ...
                .map((item) => {
                  ...
                  if (normalized.startsWith("user_")) {
                    ...
                    const user = allUsers.find(
                      (entry: any) => String(entry?.id || "").trim() === userId,
                    );
                  ...
                  if (normalized.startsWith("role_")) {
                    ...
                    const role = allRoles.find(
                      (entry: any) => String(entry?.id || "").trim() === roleId,
                    );
                    return role?.title
                      ? `نقش: ${role.title}`
                      : `نقش: ${roleId}`;
                  ...
                  const user = allUsers.find(
                    (entry: any) =>
                      String(entry?.id || "").trim() === normalized,
                  );
                  if (user)
                    return user?.display_name || user?.full_name || normalized;
                  const role = allRoles.find(
                    (entry: any) =>
                      String(entry?.id || "").trim() === normalized,
                  );
              ...
              ? field.type === FieldType.TAGS
                ? Array.isArray(rawValue)
                  ? rawValue
                      ...
                      .join("، ")
                  : String(
                      tagLabelById.get(String(rawValue ?? "").trim()) ||
                        getSingleOptionLabel(
                          field,
                          rawValue,
                          dynamicOptions,
        ...
        const operatorLabel =
          operatorLabels[
            String(
              simpleFilter?._displayOperator || simpleFilter?.operator || "eq",
            )
          ] ||
          String(
            simpleFilter?._displayOperator || simpleFilter?.operator || "eq",
          ...
          onRemove: () => {
            setViewFiltersState((prev) => {
              const nextFilters = prev.filter(
                (_item, filterIndex) => filterIndex !== index,
              );
  ...
  const columnFilterBubbles = useMemo(() => {
    ...
    return Object.entries(columnFilters || {}).flatMap(([fieldKey, values]) => {
      ...
      if (fieldKey === "assignee_id") {
        return values
          ...
          .map((rawValue) => {
            const user = allUsers.find(
              (item: any) => String(item?.id || "") === rawValue,
            );
            const role = allRoles.find(
              (item: any) => String(item?.id || "") === rawValue,
            );
            const label =
              user?.display_name || user?.full_name || role?.title || rawValue;
            return {
              id: `column:${fieldKey}:${rawValue}`,
              label: `مسئول: ${label}`,
              onRemove: () => {
                const nextFilters = { ...columnFilters };
                const current = Array.isArray(nextFilters[fieldKey])
  ...
  useEffect(() => {
    ...
    const defaultField =
      availableGroupFields.find((f) => f.type === FieldType.STATUS) ||
      availableGroupFields[0];
    setKanbanGroupBy(defaultField.key);
  }, [viewMode, kanbanGroupBy, availableGroupFields]);

  useEffect(() => {
    if (viewMode !== ViewMode.CALENDAR) return;
    const selectedFieldStillAvailable = availableCalendarFields.some(
      (field) => field.key === calendarDateField,
    );
    if (calendarDateField && selectedFieldStillAvailable) return;
    if (availableCalendarFields.length === 0) return;
    const defaultField =
      availableCalendarFields.find(
        (field) => field.key === "due_date" || field.key === "invoice_date",
  ...
  const handleSyncVoipCalls = useCallback(async () => {
    ...
    try {
      ...
      if (!data?.success) {
        throw new Error(
          String(data?.message || "همگام‌سازی تماس‌های VoIP ناموفق بود."),
        );
      }
      showListMessage(
        "success",
        String(data?.message || "تماس‌های اخیر VoIP همگام‌سازی شد."),
      );
      await tableQueryResult.refetch();
    } catch (error: any) {
      showListMessage(
        "error",
        toFaErrorMessage(error, "خطا در همگام‌سازی تماس‌های VoIP"),
      );
    } finally {
  ...
  const openRecordFromList = useCallback(
    (record: any) => {
      const recordId = String(record?.id || "").trim();
      ...
      if (resolvedModuleId === "cash_bank_operations") {
        ...
        if (sourceNavigation) {
          navigate(
            `/${sourceNavigation.moduleId}/${sourceNavigation.recordId}`,
            {
  ...
  const handleTableRowProps = useCallback(
    (record: any) => ({
  ...
  const moduleListNavigate = useCallback(
    (path: string) => {
      ...
      if (resolvedModuleId === "cash_bank_operations") {
        ...
        if (normalizedPath.startsWith(moduleRecordPrefix)) {
          ...
          if (recordId && recordId !== "create") {
            const cashBankRecord = enrichedData.find(
              (item: any) => String(item?.id || "").trim() === recordId,
            );
            const sourceNavigation =
              resolveCashBankSourceNavigation(cashBankRecord);
            if (sourceNavigation) {
              navigate(
                `/${sourceNavigation.moduleId}/${sourceNavigation.recordId}`,
                {
  ...
  function buildColumnCrudFilters(
    nextColumnFilters: ColumnFiltersState,
  ): CrudFilters {
    return buildModuleListColumnCrudFilters(
      moduleConfig,
      nextColumnFilters as any,
    );
  }
  ...
  function buildMergedFilters(
    ...
    const mergedFilters = [...nextPermFilters, ...nextViewFilters]
      ...
      .filter(Boolean)
      .filter(
        (item: any) => !getTagViewFilterMeta(item, moduleConfig),
      ) as CrudFilters;
    ...
    const searchFieldKeys = nextSearchFieldPermissions
      ...
      : moduleListSearchFieldKeys;
    const searchFilter = buildModuleListSearchFilter(
      nextSearchTerm,
      searchFieldKeys,
    );
    if (searchFilter) {
  ...
  async function buildViewCrudFilters(
    nextViewFiltersConfig: any[],
    logic?: "and" | "or",
  ): Promise<CrudFilters> {
    ...
    const resolveAssigneeTargets = async (rawValue: any) => {
      ...
      const userIdSet = new Set(
        users.map((user) => String(user?.id || "").trim()).filter(Boolean),
      );
      ...
      const needsCurrentUser =
        normalizedValues.includes("__current_user__") ||
        normalizedValues.includes("__current_role__");
      if (needsCurrentUser) {
        ...
        if (currentViewerUserId) {
          const meInDir = users.find(
            (u) => String(u?.id || "") === currentViewerUserId,
          );
          currentViewerRoleId = meInDir?.role_id
            ? String(meInDir.role_id)
            : null;
        }
      }
    ...
    for (const rawFilter of nextViewFiltersConfig) {
      ...
      if (fieldKey === WORKFLOW_ASSIGNEE_FIELD_KEY) {
        ...
        const { assigneeIds, assigneeRoleIds } =
          await resolveAssigneeTargets(value);
      ...
      const field = moduleConfig.fields.find(
        (item) => String(item?.key || "").trim() === fieldKey,
      );
      if (!field) continue;
      ...
      if (field.type === FieldType.TAGS) {
        const isCountOperator =
          operator === "multi_count_gt" || operator === "multi_count_lt";
        ...
        filters.push({
          ...
          value: {
            ...
            selectedCount:
              isCountOperator && Number.isFinite(selectedCount)
      ...
      if (isJsonArrayViewFilterField(field)) {
        if (operator === "multi_count_gt" || operator === "multi_count_lt") {
          ...
          const { data: matchedRows, error } = await supabase.rpc(
            "filter_records_by_json_array_count",
            {
        ...
        const arrayFilters = buildJsonArrayViewCrudFilters(
          fieldKey,
          operator,
          value,
        );
      ...
      switch (operator) {
        ...
        case "is_tomorrow": {
          ...
          if (operator === "is_tomorrow")
            baseDate.setDate(baseDate.getDate() + 1);
          filters.push(
            {
              field: fieldKey,
              operator: "gte",
              value: buildViewDateBoundaryValue(field, baseDate, "start"),
            } as any,
            {
              field: fieldKey,
              operator: "lte",
              value: buildViewDateBoundaryValue(field, baseDate, "end"),
            } as any,
          );
          continue;
        }
  ...
  async function buildPermissionViewCrudFilters(
    group?: ViewConditionGroup | null,
  ): Promise<CrudFilters> {
    ...
    const allFilters = await buildViewCrudFilters(
      conditionsAll as any[],
      "and",
    );
  ...
  function applyCombinedFilters(
    ...
    const mergedFilters = buildMergedFilters(
      ...
      nextPermissionFilters,
      nextFieldPermissions ??
        (fieldPermissionsReady ? fieldPermissions : undefined),
    );
  ...
  useEffect(() => {
    ...
    const loadTagFilteredRows = async () => {
      try {
        ...
        const fromIndex = (safeCurrent - 1) * safePageSize;
        const pageIds = orderedFilteredIds.slice(
          fromIndex,
  ...
  const handleViewChange = useCallback(
    (view: SavedView | null, config: any) => {
  ...
  const handleTableChange = useCallback(
    (pagination: any, _tableFilters: any, sorter: any, extra: any) => {
      ...
      if (extra?.action === "sort") {
        const nextSorters = normalizeCrudSorters(
          mapAntdSorterToCrudSorting(sorter),
        );
        setSorters(
          ensureStableCrudSorters(
            nextSorters.length > 0
              ? nextSorters
              : getDefaultSorters(moduleConfig),
          ),
        );
        return;
      }
  ...
  const handleSelectAllAcrossPages = useCallback(async () => {
    ...
    const closeLoadingMessage = showListMessage(
      "loading",
      "در حال انتخاب همه رکوردهای صفحات...",
    );
    ...
    try {
      ...
      setSelectedRowKeys((prev) =>
        Array.from(new Set([...prev, ...allowedKeys])),
      );
      setSelectedRowsMap((prev) => {
        const nextMap = { ...prev };
      ...
      showListMessage(
        "success",
        `${allowedKeys.length} رکورد از همه صفحات انتخاب شد.`,
      );
    } catch (error: any) {
      showListMessage(
        "error",
        toFaErrorMessage(error, "انتخاب همه رکوردهای صفحات ناموفق بود."),
      );
    } finally {
  ...
  const handleBulkDelete = () => {
    ...
    if (hasLockedSelectedRows) {
      showListMessage(
        "warning",
        "در میان رکوردهای انتخاب‌شده، رکورد قفل‌شده وجود دارد و قابل حذف نیست.",
      );
      return;
    }
    ...
    modal.confirm({
      ...
      onOk: async () => {
        ...
        try {
          ...
          showListMessage(
            "error",
            toFaErrorMessage(error, "حذف رکوردها ناموفق بود."),
          );
        }
      },
    });
  ...
  const handleBulkRecordLockToggle = useCallback(async () => {
    const recordIds = Array.from(
      new Set(
        selectedRowKeys.map((key) => String(key || "").trim()).filter(Boolean),
      ),
    );
    if (!resolvedModuleId || recordIds.length === 0 || bulkRecordLockSaving)
      return;

    setBulkRecordLockSaving(true);
    try {
      // وضعیت تازه از سرور خوانده می‌شود تا انتخاب بین چند صفحه یا تغییر هم‌زمان، تصمیم نادرست نسازد.
      const latestLocks = await fetchRecordLockMap(
        resolvedModuleId,
        recordIds,
        { forceRefresh: true, throwOnError: true },
      );
      const shouldUnlock = recordIds.every((recordId) =>
        latestLocks.has(recordId),
      );
      if (shouldUnlock && !canUnlockRecords) {
        showListMessage(
          "warning",
          "اجازه باز کردن قفل رکوردهای انتخاب‌شده را ندارید.",
        );
        return;
      }
      if (!shouldUnlock && !canLockRecords) {
        showListMessage(
          "warning",
          "اجازه قفل کردن رکوردهای انتخاب‌شده را ندارید.",
        );
        return;
      }
      ...
      const refreshedLocks = await fetchRecordLockMap(
        resolvedModuleId,
        recordIds,
        { forceRefresh: true, throwOnError: true },
      );
      ...
      showListMessage(
        "success",
        shouldUnlock
          ? "قفل رکوردهای انتخاب‌شده باز شد."
          : "رکوردهای انتخاب‌شده قفل شدند.",
      );
    } catch (error: any) {
      showListMessage(
        "error",
        toFaErrorMessage(error, "تغییر وضعیت قفل رکوردها ناموفق بود."),
      );
    } finally {
      setBulkRecordLockSaving(false);
    }
  }, [
    bulkRecordLockSaving,
    canLockRecords,
    canUnlockRecords,
    resolvedModuleId,
    selectedRowKeys,
  ...
  const handleBulkEditOpen = () => {
    if (hasLockedSelectedRows) {
      showListMessage(
        "warning",
        "در میان رکوردهای انتخاب‌شده، رکورد قفل‌شده وجود دارد و قابل ویرایش نیست.",
      );
      return;
    }
  ...
  const fetchSelectedRecords = useCallback(async () => {
    if (!selectedRowKeys.length || !moduleConfig || !resolvedModuleId)
      return [];
    const tableName = moduleConfig.table || resolvedModuleId;
    const ids = Array.from(
      new Set(selectedRowKeys.map((id) => String(id).trim()).filter(Boolean)),
    );
    const rowChunks: any[][] = [];
    for (
      let index = 0;
      index < ids.length;
      index += SELECTED_RECORD_FETCH_CHUNK_SIZE
    ) {
  ...
  const handleMergeOpen = useCallback(async () => {
    ...
    try {
      ...
      showListMessage(
        "error",
        toFaErrorMessage(error, "آماده‌سازی ادغام ناموفق بود."),
      );
    } finally {
    ...
    async (
      ...
      try {
        const relationReferences =
          findModuleRelationReferences(resolvedModuleId);
        const { error: relationError } = await supabase.rpc(
          "merge_module_record_references",
          {
        ...
        const { error: mergeActivityError } = await supabase.rpc(
          "log_module_record_merge",
          {
            p_module_id: resolvedModuleId,
            p_survivor_id: meta.survivorId,
            p_duplicate_ids: meta.duplicateIds,
        ...
        if (isRecycleBinEnabledModule(resolvedModuleId)) {
          await moveModuleRecordsToRecycleBin(
            resolvedModuleId,
            meta.duplicateIds,
          );
        } else {
        ...
        showListMessage(
          "error",
          toFaErrorMessage(error, "ادغام رکوردها ناموفق بود."),
        );
      } finally {
        hide?.();
  ...
  const handleBulkSmsOpen = useCallback(async () => {
    ...
    try {
      ...
      if (!recipients.length) {
        showListMessage(
          "warning",
          "در رکوردهای انتخاب‌شده شماره موبایل معتبر پیدا نشد.",
        );
        return;
      }
      ...
      showListMessage(
        "error",
        toFaErrorMessage(error, "ارسال پیامک گروهی ناموفق بود."),
      );
    }
  }, [
    canBulkSendSms,
  ...
  const handleBuildRecordFromSelection = useCallback(
    async (values: any) => {
      ...
      const payload =
        bulkBuildTarget === "product_bundles"
          ? {
              ...values,
              products: buildPackageItemsFromRecords(
                selectedRecords,
                bulkBuildSourceModule,
              ),
            }
          : {
              ...values,
              items: buildPriceListItemsFromRecords(
                selectedRecords,
                bulkBuildSourceModule,
              ),
            };
      const auditedPayload = authUserId
        ? {
            ...
            updated_by: payload.updated_by ?? authUserId,
          }
        : payload;
      ...
      if (
        insertResult.error &&
        (errorText.includes("created_by") || errorText.includes("updated_by"))
      ) {
  ...
  const handleExportExcel = useCallback(async () => {
    ...
    try {
      ...
      const { buildListPrintableFields, formatListCellValue } =
        await import("../utils/listPrintExport");
      const XLSX = await import("xlsx");
      const exportFields = buildListPrintableFields(
        moduleConfig,
        canViewField,
        visibleColumns,
        dynamicOptions,
      ...
      sheet["!cols"] = exportFields.map((field) => ({
        wch: Math.min(48, Math.max(14, String(field.label || "").length + 4)),
      }));
      ...
      showListMessage(
        "error",
        toFaErrorMessage(error, "خروجی گرفتن ناموفق بود."),
      );
    } finally {
      hide();
    }
  ...
  const handleExportPrint = useCallback(async () => {
    ...
    const hide = showListMessage(
      "loading",
      "در حال آماده‌سازی پیش‌نمایش چاپ...",
      0,
    );
    try {
      ...
      showListMessage(
        "error",
        toFaErrorMessage(error, "آماده‌سازی چاپ ناموفق بود."),
      );
    } finally {
      hide();
    }
      ...
      onOk: async () => {
        ...
        try {
          ...
          if (
            ...
            resolvedModuleId === "process_templates" ||
            resolvedModuleId === "web_forms"
          ) {
            let copiedCount = 0;
            for (let idx = 0; idx < records.length; idx += 1) {
              const record = records[idx];
              const payload = buildCopyPayload(record, {
                nameField,
                copyIndex: idx,
                moduleId: resolvedModuleId,
              });
              ...
              if (inserted?.id) {
                if (resolvedModuleId === "production_orders") {
                  await copyProductionOrderRelations(
                    supabase,
                    String(record.id),
                    String(inserted.id),
                  );
                } else if (resolvedModuleId === "process_templates") {
                  await copyProcessTemplateStagesRelations(
                    supabase,
                    String(record.id),
                    String(inserted.id),
                  );
                } else {
                  await copyWebFormFieldsRelations(
                    supabase,
                    String(record.id),
                    String(inserted.id),
                  );
          ...
          showListMessage(
            "error",
            toFaErrorMessage(err, "کپی رکوردها ناموفق بود."),
          );
        } finally {
          hide();
        }
  ...
  const handleCopyViaCreateForm = () => {
    ...
    if (selectedRowKeys.length > 1) {
      showListMessage(
        "warning",
        "برای کپی از طریق فرم، فقط یک رکورد را انتخاب کنید.",
      );
      return;
    }
    modal.confirm({
      ...
      onOk: async () => {
        ...
        try {
          ...
          showListMessage(
            "error",
            toFaErrorMessage(err, "آماده‌سازی کپی ناموفق بود."),
          );
        } finally {
          hide();
        }
  ...
  const handleCreateGroupOrderFromSelection = () => {
    ...
    if (!allSelectedPendingInProductionOrders) {
      showListMessage(
        "error",
        "برای ایجاد سفارش گروهی، همه سفارش‌های انتخاب‌شده باید در وضعیت «در انتظار» باشند.",
      );
      return;
    }
  ...
  const handleBulkSave = async (
    ...
    if (
      ...
      showListMessage(
        "warning",
        "برای گزارش‌های سیستمی فقط برچسب‌ها قابل ویرایش هستند.",
      );
      return;
    }
    ...
    const normalizedChanges =
      Object.keys(changes).length > 0 && resolvedModuleId === "tasks"
        ? attachTaskCompletionIfNeeded(changes)
        : changes;
    const shouldUpdateRecordPayload =
      Object.keys(normalizedChanges).length > 0 && !isSystemManagedModule;
    const selectedIds = selectedRowKeys.map((id) => String(id)).filter(Boolean);
    if (!selectedIds.length) return;
    if (
      selectedIds.some((selectedId) =>
        isRecordIdLocked(selectedId, selectedRowsMap[selectedId]),
      )
    ) {
      showListMessage(
        "warning",
        "در میان رکوردهای انتخاب‌شده، رکورد قفل‌شده وجود دارد و قابل بروزرسانی نیست.",
      );
      return;
    }
    ...
    const hide = showListMessage(
      "loading",
      "در حال بروزرسانی موارد انتخاب‌شده...",
      0,
    );
    try {
      ...
      showListMessage(
        "error",
        toFaErrorMessage(error, "بروزرسانی رکورد ناموفق بود."),
      );
    } finally {
      hide?.();
    }
  ...
  const handleKanbanRecordMove = useCallback(
    async (record: any, targetColumnKey: string) => {
      ...
      const targetOption = groupField.options?.find(
        (option) => String(option?.value ?? "") === String(targetColumnKey),
      );
      const nextValue = targetOption?.value ?? targetColumnKey;
      if (String(record?.[kanbanGroupBy] ?? "") === String(nextValue ?? ""))
        return;
  ...
  const moduleActionItems: MenuProps["items"] = useMemo(() => {
    ...
    if (
      resolvedModuleId === "reservations" &&
      canEditModule &&
      fieldPermissions.__action_reservation_settings !== false
    ) {
      items.push({
    ...
    if (
      resolvedModuleId === "invoices" &&
      retailSalesInvoiceEnabled &&
      canEditModule &&
      fieldPermissions.__action_retail_invoice_settings !== false
    ) {
      items.push({
        key: "retail_invoice_settings",
        icon: <SettingOutlined />,
        label: "تنظیمات فاکتور فروشگاهی",
      });
    }
  ...
  const hasModuleActionItems =
    Array.isArray(moduleActionItems) && moduleActionItems.length > 0;
  ...
  const handleModuleActionClick: MenuProps["onClick"] = ({ key }) => {
    ...
    if (key === "retail_invoice_settings") {
      setIsRetailInvoiceSettingsOpen(true);
      return;
    }
    if (key === "web_forms") {
    ...
    <div className="module-list-page box-border p-3 md:p-5 max-w-[1800px] mx-auto pb-20 md:pb-5 h-full min-h-0 flex flex-col overflow-hidden">
      <div className="flex flex-col gap-0 mb-0.5 md:mb-1 shrink-0">
        ...
        <div className="flex flex-wrap items-start justify-between gap-2">
          ...
          {selectedRowKeys.length > 0 ||
          (isMobileViewport && canShowGoalCards && !isSystemManagedModule) ? (
            <div className="order-last basis-full pt-0 min-w-0 md:order-none md:mt-0 md:basis-auto md:pt-0 md:flex md:flex-[0_1_420px] md:items-center md:justify-start md:self-center xl:flex-[0_1_460px]">
              <div
                ...
                style={
                  selectedRowKeys.length > 0 && utilitySlotHeight
                    ? { minHeight: `${utilitySlotHeight}px` }
                    : undefined
                }
              >
                {selectedRowKeys.length > 0 ? (
                  <BulkActionsBar
                    ...
                    onSelectAllPages={
                      visibleSelectableRowKeys.length > 0
                    ...
                    onDelete={
                      selectedRowKeys.length &&
                      canDeleteModule &&
                      (!isSystemManagedModule || allowSystemManagedDelete)
                        ? handleBulkDelete
                        : undefined
                    }
                    onExport={selectedRowKeys.length ? handleExport : undefined}
                    exportMenuItems={
                      selectedRowKeys.length ? exportMenuItems : undefined
                    }
                    extraActions={[
                      ...(selectedRowKeys.length > 0 &&
                      (allSelectedRowsLocked
                        ? canUnlockRecords
                        : canLockRecords)
                        ? [
                            {
                              key: "toggle_record_locks",
                              icon: allSelectedRowsLocked ? (
                                <UnlockOutlined />
                              ) : (
                                <LockOutlined />
                              ),
                              tooltip: allSelectedRowsLocked
                                ? "باز کردن قفل انتخاب‌ها"
                                : "قفل کردن انتخاب‌ها",
                              onClick: handleBulkRecordLockToggle,
                              disabled:
                                bulkRecordLockSaving || recordLockMapLoading,
                            },
                          ]
                        : []),
                        ...
                        ? [
                            {
                              ...
                              tooltip: "ساخت پکیج",
                              onClick: () =>
                                setBulkBuildTarget("product_bundles"),
                            },
                            {
          ...
          {selectedRowKeys.length === 0 && (
            <div className="flex items-center gap-2 shrink-0">
              ...
              {canCreateModule && (
                ...
                >
                  افزودن
                  <kbd className="keyboard-shortcut-hint mr-1 text-[9px] opacity-70">
                    Ctrl+N
                  </kbd>
                </Button>
              )}
              ...
              {mobileViewModeOptions.map((option) => {
                ...
                return (
                  <button
                    ...
                    onClick={() =>
                      handleMobileViewModeSelect(option.value as ViewMode)
                    }
                  >
                    ...
                    {isActive ? (
                      <span className="shrink-0 rounded-full bg-leather-600 px-2 py-0.5 text-[10px] font-bold text-white">
                        فعال
                      </span>
                    ) : null}
                  </button>
              ...
              <div className="rounded-2xl border border-gray-200 bg-white px-3 py-3 dark:border-white/10 dark:bg-[#171717]">
                ...
                <div className="space-y-2">
                  {kanbanGroupOptions.map((option) => {
                    const isActive =
                      (kanbanGroupBy || kanbanGroupOptions[0]?.value) ===
                      option.value;
                    return (
                      <button
                        key={option.value}
              ...
              <div className="rounded-2xl border border-gray-200 bg-white px-3 py-3 dark:border-white/10 dark:bg-[#171717]">
                ...
                <div className="space-y-2">
                  {calendarDateFieldOptions.map((option) => {
                    const isActive =
                      (calendarDateField ||
                        calendarDateFieldOptions[0]?.value) === option.value;
                    return (
                      <button
                        key={option.value}
                        type="button"
            ...
            <div className="flex h-full items-center gap-1 overflow-x-auto px-0.5 no-scrollbar">
              ...
              {allListFilterBubbles.length > 0 ? (
                <Button
                  type="link"
                  size="small"
                  className="!h-6 !px-1 shrink-0 text-[10px]"
                      ...
                      <Button
                        ...
                        onClick={() =>
                          setGridPageSize((prev) =>
                            Math.min(prev + gridLoadStep, enrichedData.length),
                          )
                        }
      ...
      {isBulkEditOpen && (
        <React.Suspense fallback={null}>
          <SmartForm
            module={
              isSystemManagedModule && isBulkEditMode
                ? tagOnlyBulkEditModule || moduleConfig
                : moduleConfig
            }
            visible={isBulkEditOpen}
            recordId={
              isSystemManagedModule ? undefined : editRecordId || undefined
            }
            onCancel={() => {
              setIsBulkEditOpen(false);
              setEditRecordId(null);
            ...
            title={
              isBulkEditMode
                ? `ویرایش گروهی ${selectedRowKeys.length} مورد`
                : `ویرایش مورد انتخابی`
            }
        ...
        <React.Suspense fallback={null}>
          <DeleteModuleRecordsModal
            ...
            onDeleted={async () => {
              ...
              showListMessage(
                "success",
                "رکوردهای انتخاب‌شده به سطل بازیافت منتقل شدند.",
              );
              tableQueryResult.refetch();
            }}
          />
      ...
      {isBillboardStatusChangeOpen &&
      (resolvedModuleId === "billboards" ||
        resolvedModuleId === "billboard_status_changes") ? (
        <React.Suspense fallback={null}>
      ...
      {resolvedModuleId === "invoices" && retailSalesInvoiceEnabled ? (
        <React.Suspense fallback={null}>
          <RetailInvoiceSettingsModal
            open={isRetailInvoiceSettingsOpen}
            onClose={() => setIsRetailInvoiceSettingsOpen(false)}
          />
        </React.Suspense>
      ) : null}
      ...
      >
        <AssistantPanel
          active={isListAiDrawerOpen}
          initialPrompt={listAiInitialPrompt}
        ...
        <React.Suspense fallback={null}>
          <RelatedRecordPopover
            ...
            hideFullRecordAction={
              moduleConfig?.hideFullRecordAction ?? detailDisabled
            }
