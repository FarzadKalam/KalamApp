...
const SaasAdminCatalogItems: React.FC = () => {
  ...
  const entitlementOptions = kind === 'module'
    ? moduleOptions
    : kind === 'feature'
      ? SAAS_FEATURE_OPTIONS
      : kind === 'bundle'
        ? [{ id: editing?.entitlement_code || 'bundle', label: editing?.title || 'بسته ترکیبی' }]
        : quotaOptions;
  ...
  return <div className="p-4 md:p-6 space-y-4">
    ...
    <Card className="rounded-2xl"><Table loading={loading} rowKey="id" dataSource={items} scroll={{ x: 760 }} pagination={{ pageSize: 20 }} locale={{ emptyText: 'هنوز افزونه‌ای در کاتالوگ قیمت‌گذاری نشده است؛ از «افزودن افزونه» یک قلم فعال بسازید.' }} columns={[
      ...
      { title: 'نوع', dataIndex: 'item_kind', render: (value) => <Tag>{value === 'bundle' ? 'بسته' : value === 'module' ? 'ماژول' : value === 'feature' ? 'امکان' : 'سهمیه'}</Tag> },
