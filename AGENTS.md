...
### ۰.۱. مبنای بررسی و دسترسی عملیاتی
- پیش از بررسی یا اصلاح هر تابع، رفتار یا منطق، ابتدا آخرین تغییرات همان بخش (شامل تغییرات محلی، commitهای اخیر، migrationها و تغییرات وابسته) را بررسی کن؛ نتیجه‌گیری بر مبنای نسخه‌های قدیمی یا صرفاً نام فایل ممنوع است.
- Agent با کلید deploy به سرور دسترسی دارد و می‌تواند لاگ‌های واقعی را برای عیب‌یابی بخواند. اجرای استقرار frontend، Edge Function و migration فقط پس از تأیید صریح کاربر در همان گفتگو مجاز است؛ پیش از هر استقرار، build و بررسی متناسب با تغییر را انجام بده و نتیجه را گزارش کن.

### ۰.۲. انتشار PWA و جلوگیری از نسخهٔ قدیمی
- در هر انتشار frontend، نسخهٔ `package.json`، `package-lock.json` و `.version-changes.json` باید هم‌راستا باشند و پس از build، `dist/sw.js` باید cache version مهرشدهٔ همان نسخه و بدون token خام داشته باشد؛ `dist/version.json` نیز باید همان نسخه را اعلام کند. اسکریپت deploy این کنترل را قبل از بسته‌بندی اجباری می‌کند.
- پس از استقرار frontend، روی دامنهٔ واقعی PWA بررسی کن که `sw.js`، `index.html` و `version.json` با `Cache-Control: no-cache, no-store, must-revalidate` عرضه می‌شوند و `sw.js` نسخهٔ تازه را دارد. بدون این بررسی، انتشار PWA کامل تلقی نمی‌شود.
- ثبت Service Worker باید به نسخهٔ build گره بخورد و هنگام جایگزین‌شدن کنترلر قدیمی، فقط یک‌بار شِل قدیمی برنامه را تازه کند؛ نصب نخست یا کار آفلاین کاربر نباید حلقهٔ reload ایجاد کند.
...
### ارسال در KalamApp / کلام‌اپ
- وقتی کاربر گفت «این را در KalamApp / کلام‌اپ بفرست»، منظور ارسال محتوا و فایل‌ها به‌صورت چند یادداشت یا پیام سیستمی داخلی است.
- مقصد پیش‌فرض این درخواست‌ها، حساب «مجتبی جعفری» با ایمیل `fj.lyric@gmail.com` است؛ پیش از ارسال، کاربر و سازمان او را با یک query محدود و دقیق تأیید کن.
- پیام‌های سیستمی باید فقط برای همین گیرنده scope شوند؛ هرگز org-wide نباشند و شناسهٔ خام کاربر در متن پیام یا UI نمایش داده نشود.
- متن و فایل‌ها را در چند پیام کوتاه، عنوان‌دار و قابل پیگیری ارسال کن و پس از ثبت، وجود پیام‌ها و پیوست‌ها را بررسی کن.
