-- =====================================================
-- KalamApp - Phase 175: Fix assign_system_code trigger — add SECURITY DEFINER
-- Date: 2026-05-22
-- Type: Bug fix / idempotent
-- Problem:
--   Phase 163 (tenant_isolation_hardening) revoked ALL access to
--   public.system_code_counters from authenticated/anon/public roles.
--   However, assign_system_code_from_module_settings() is a BEFORE INSERT
--   trigger declared WITHOUT security definer — so it runs in the context
--   of the authenticated user and cannot INSERT/UPDATE system_code_counters.
--   Result: every INSERT into any table with a system_code trigger
--   (invoices, marketing_leads, products, employees, customers, ...) fails
--   with "permission denied for table system_code_counters" → HTTP 403.
--
-- Fix:
--   Recreate the function as SECURITY DEFINER so it runs as the function
--   owner (postgres) and can access system_code_counters regardless of
--   the calling user's grants.
-- =====================================================
...
do $$
begin
  raise notice 'Phase 175: assign_system_code_from_module_settings() now SECURITY DEFINER.';
  raise notice 'INSERT into invoices, marketing_leads, products, employees, etc. should work again.';
end;
$$;
