...
create table if not exists public.company_settings (
  ...
  website text,
  email text,
  qr_scan_enabled boolean not null default false,
  logo_url text,
  icon_url text,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
...
create trigger trg_chat_groups_updated_at before update on public.chat_groups
...
create policy p_chat_groups_auth_all on public.chat_groups for all to authenticated using (true) with check (true);
