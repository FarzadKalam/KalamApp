-- تاریخ مانده اول دوره از زمان ایجاد خود رکورد مخاطب گرفته می‌شود؛ ترتیب آن همچنان اول فهرست است.
...
begin
  ...
  select jsonb_strip_nulls(jsonb_build_object('company_name', coalesce(company_full_name, company_name, trade_name), 'trade_name', trade_name, 'logo_url', logo_url, 'currency_label', currency_label)) into v_company from public.company_settings where org_id = v_card.org_id order by updated_at desc limit 1;
  with raw_rows as (
    ...
    from (
      ...
      union all select id, created_at, previous_system_balance_total from public.employees where v_card.entity_type = 'employee' and id = v_card.entity_id and org_id = v_card.org_id
    ) c where coalesce(c.previous_system_balance_total, 0) <> 0
    union all
    select 'invoice', case when v_card.entity_type = 'customer' then 'صدور فاکتور فروش' else 'ثبت فاکتور خرید' end, i.status, null, i.invoice_date, case when v_card.entity_type = 'customer' then i.total_invoice_amount else 0 end, case when v_card.entity_type = 'supplier' then i.total_invoice_amount else 0 end, concat(case when v_card.entity_type = 'customer' then 'فاکتور فروش' else 'فاکتور خرید' end, case when coalesce(i.remaining_balance, 0) <> 0 then ' | مانده: ' || i.remaining_balance else '' end), i.created_at, 'invoice_' || i.id::text
    from (select id, invoice_date, status, total_invoice_amount, remaining_balance, created_at from public.invoices where v_card.entity_type = 'customer' and customer_id = v_card.entity_id and org_id = v_card.org_id and status in ('confirmed','final','settled','completed') union all select id, invoice_date, status, total_invoice_amount, remaining_balance, created_at from public.purchase_invoices where v_card.entity_type = 'supplier' and supplier_id = v_card.entity_id and org_id = v_card.org_id and status in ('confirmed','final','settled','completed')) i
    union all
    select case when o.operation_type = 'payment' then 'payment' else 'receipt' end, 'عملیات نقد و بانک', o.status, o.payment_type, o.operation_date, case when v_card.entity_type = 'customer' and o.operation_type = 'receipt' then abs(o.amount) when v_card.entity_type <> 'customer' and o.operation_type = 'payment' then abs(o.amount) else 0 end, case when v_card.entity_type = 'customer' and o.operation_type = 'payment' then abs(o.amount) when v_card.entity_type <> 'customer' and o.operation_type = 'receipt' then abs(o.amount) else 0 end, coalesce(o.description, ''), o.created_at, 'operation_' || o.id::text
    from public.cash_bank_operations o where o.org_id = v_card.org_id and o.status in ('received','approved','paid','settled','cleared') and o.operation_type <> 'transfer' and ((v_card.entity_type = 'customer' and o.customer_id = v_card.entity_id) or (v_card.entity_type = 'supplier' and o.supplier_id = v_card.entity_id) or (v_card.entity_type = 'employee' and o.employee_id = v_card.entity_id))
    union all
    select case when v_card.entity_type = 'customer' then 'receipt' else 'payment' end, case when v_card.entity_type = 'customer' then 'دریافت فاکتور فروش' else 'پرداخت فاکتور خرید' end, coalesce(p.item ->> 'status', 'received'), coalesce(p.item ->> 'payment_type', ''), coalesce(nullif(p.item ->> 'date', '')::date, i.invoice_date), case when v_card.entity_type = 'customer' then abs((p.item ->> 'amount')::numeric) else 0 end, case when v_card.entity_type = 'supplier' then abs((p.item ->> 'amount')::numeric) else 0 end, coalesce(p.item ->> 'description', ''), i.created_at, 'legacy_payment_' || i.id::text || '_' || p.ordinality::text
    ...
    where lower(coalesce(p.item ->> 'status', 'received')) in ('received','paid','approved','cleared') and coalesce(p.item ->> 'amount', '') ~ '^-?[0-9]+(\.[0-9]+)?$' and not exists (select 1 from public.cash_bank_operations o where o.org_id = v_card.org_id and ((v_card.entity_type = 'customer' and o.sales_invoice_id = i.id) or (v_card.entity_type = 'supplier' and o.purchase_invoice_id = i.id)))
