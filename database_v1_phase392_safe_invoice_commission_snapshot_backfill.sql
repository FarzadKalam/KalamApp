-- جایگزین امن backfill فاز ۳۸۸
-- رکوردهای قفل‌شده عمداً تغییر نمی‌کنند؛ قفل شدن فاکتور باید سابقهٔ آن را
-- غیرقابل‌ویرایش نگه دارد. فاکتورهای عادی همچنان کلید و نرخ snapshot می‌گیرند.
...
begin
  ...
  loop
    ...
    -- صفر snapshot نمی‌شود: اگر در زمان ساخت فاکتور درصدی تعیین نشده باشد،
    -- نخستین محاسبهٔ واقعی می‌تواند نرخ درست را تثبیت کند.
    if nullif(trim(v_item->>'commission_percentage_snapshot'), '') is null then
      ...
      if coalesce(nullif(trim(v_item->>'commission_percentage'), '') ~ '^[0-9]+([.][0-9]+)?$', false)
        ...
        where id = v_product_id and org_id = new.org_id;
...
begin
  ...
  if v_new_status = 'settled' and (tg_op = 'INSERT' or v_old_status <> 'settled') then
    new.settled_at := coalesce(new.settled_at, now());
  end if;
  if v_new_status = 'completed' and (tg_op = 'INSERT' or v_old_status <> 'completed') then
...
-- فاکتورهای قفل‌شده (مثلاً دارای سند یا فیش نهایی) نباید صرفاً برای تکمیل
-- snapshot تغییر کنند. این شرط برای هر backfill پایین هم تکرار شده است.
update public.invoices invoice
set "invoiceItems" = invoice."invoiceItems"
where jsonb_array_length(coalesce(invoice."invoiceItems", '[]'::jsonb)) > 0
  and not exists (
    select 1 from public.record_locks lock_row
    where lock_row.org_id = invoice.org_id
      and lock_row.record_id = invoice.id
      and (lock_row.module_id = 'invoices' or lock_row.metadata ->> 'table_name' = 'invoices')
  );
...
where lower(coalesce(status, '')) in ('confirmed', 'final', 'prepayment', 'settled', 'completed')
  ...
  and not exists (
    select 1 from public.record_locks lock_row
    where lock_row.org_id = invoice.org_id and lock_row.record_id = invoice.id
      and (lock_row.module_id = 'invoices' or lock_row.metadata ->> 'table_name' = 'invoices')
  );
