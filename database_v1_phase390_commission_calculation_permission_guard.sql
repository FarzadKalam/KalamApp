-- محدود کردن ثبت اتمیک پورسانت به کاربران مجاز حقوق و دستمزد
-- تعریف قبلی تابع حفظ می‌شود و فقط کنترل دسترسی پیش از هر تغییری افزوده می‌شود.
...
begin
  ...
  for v_draft in select value from jsonb_array_elements(p_draft_payloads) as item(value) loop
    ...
    insert into public.commission_drafts (
      ...
      coalesce(v_draft->'details', '{}'::jsonb), now()
    )
    on conflict (source_key) where source_key is not null do update
    set employee_id = excluded.employee_id, assignee_id = excluded.assignee_id,
      period_start = excluded.period_start, period_end = excluded.period_end,
      source_basis = excluded.source_basis, percent_mode = excluded.percent_mode,
      eligibility_event_type = excluded.eligibility_event_type,
      eligibility_event_at = excluded.eligibility_event_at,
      invoice_id = excluded.invoice_id, invoice_item_key = excluded.invoice_item_key,
      entitled_amount = excluded.entitled_amount, posted_amount = excluded.posted_amount,
      remaining_amount = excluded.remaining_amount, decision_status = excluded.decision_status,
      ...
      draft_status = excluded.draft_status, details = excluded.details, updated_at = now()
    where public.commission_drafts.org_id = v_org_id;
  end loop;

  select id into v_ledger_id
  from public.payroll_calculation_entries
  where org_id = v_org_id and source_type = 'commission'
    and source_key = v_source_key and status <> 'voided'
  for update;

  if v_ledger_id is not null then
    if exists (select 1 from public.payroll_calculation_entries where id = v_ledger_id and status = 'included_in_payroll') then
      raise exception 'commission_already_in_payroll';
    end if;
    update public.payroll_calculation_entries
    set entry_type = coalesce(nullif(trim(p_ledger_payload->>'entry_type'), ''), entry_type),
      ...
      title = coalesce(p_ledger_payload->>'title', ''), amount = coalesce((p_ledger_payload->>'amount')::numeric, 0),
      ...
      details = coalesce(p_ledger_payload->'details', '{}'::jsonb), updated_at = now()
    where id = v_ledger_id and org_id = v_org_id;
  else
    insert into public.payroll_calculation_entries (
      org_id, employee_id, period_start, period_end, entry_type, source_type, source_key,
      source_module_id, source_record_id, title, amount, quantity, rate, status, assignee_id, details
    ) values (
      ...
      'commission', v_source_key, nullif(trim(p_ledger_payload->>'source_module_id'), ''),
      nullif(trim(p_ledger_payload->>'source_record_id'), '')::uuid,
      coalesce(p_ledger_payload->>'title', ''), coalesce((p_ledger_payload->>'amount')::numeric, 0),
  ...
  where org_id = v_org_id and source_type = 'commission' and employee_id = v_employee_id
    and period_start = v_period_start and period_end = v_period_end and id <> v_ledger_id
    and status in ('draft', 'proposed') and details->>'basis' = v_basis
