-- TazeSystem - Phase 409: پیام‌های سیستم در runtime مرکزی پیام‌رسانی V2
-- همهٔ پیام‌های سیستمی/هوش مصنوعی باید مانند پیام داخلی در inbox واحد ثبت شوند.
...
-- پیام‌های سیستمی که در فاصلهٔ ناقص بودن trigger ثبت شده‌اند نیز به همان
-- inbox مرکزی وارد می‌شوند؛ دادهٔ اصلی notes تغییر نمی‌کند.
...
select
  ...
  'notes',
  case
    when lower(trim(coalesce(note.source_type, note.metadata->>'source_type', ''))) in ('ai', 'assistant') then 'assistant'
    else 'system'
  end,
  'upsert',
  'پیام سیستم',
