-- Global search performance and permission alignment.
-- Replaces the phase122 RPC without changing its public signature.
...
declare
  ...
  v_supported_modules constant text[] := array[
    ...
    'employee_contracts', 'recruitment_applicants', 'surveys'
  ];
  v_module text;
  v_columns text[];
  v_config_keys text[];
...
begin
  ...
  loop
    if v_module !~ '^[a-zA-Z_][a-zA-Z0-9_]*$'
      or not (v_module = any(v_supported_modules)) then
      continue;
    end if;
    ...
    where a.attrelid = to_regclass(format('public.%I', v_module))
      ...
      and a.attname ~* '(name|title|code|number|phone|mobile|email|subject|description|notes|status|city|address)';
    ...
    from unnest(array['system_code', 'manual_code', 'legacy_contact_code', 'legacy_system_code', 'legacy_invoice_number', 'accounting_code', 'mobile_1', 'phone']) with ordinality item(key, ord)
    where key = any(v_columns)
      and coalesce((v_module_permissions -> 'fields' ->> key)::boolean, true);
    ...
    v_sql := format(
      'select %L::text as module_id,
              ...
              %s as payload,
              jsonb_array_length(match_data.matched_fields)::numeric as score,
              %s as created_at
       from public.%I t
...
begin
  for idx in
    select *
    from (values
      ('customers', 'idx_customers_global_search_text_v2', array['full_name', 'first_name', 'last_name', 'business_name', 'legal_name', 'mobile_1', 'mobile_2', 'phone', 'assistant_phone', 'system_code', 'legacy_contact_code', 'accounting_code', 'email', 'national_code', 'national_id', 'city', 'address', 'notes']::text[]),
      ('suppliers', 'idx_suppliers_global_search_text_v2', array['business_name', 'first_name', 'last_name', 'mobile_1', 'mobile_2', 'phone', 'system_code', 'email', 'city', 'address']::text[]),
      ('employees', 'idx_employees_global_search_text_v2', array['full_name', 'first_name', 'last_name', 'legacy_system_code', 'system_code', 'national_code', 'mobile_1', 'phone', 'job_title']::text[]),
      ('products', 'idx_products_global_search_text_v2', array['name', 'system_code', 'manual_code', 'accounting_code', 'product_identifier', 'category', 'product_type', 'description']::text[]),
      ('invoices', 'idx_invoices_global_search_text_v2', array['name', 'system_code', 'legacy_invoice_number', 'status', 'legacy_status', 'sale_source']::text[]),
      ('purchase_invoices', 'idx_purchase_invoices_global_search_text_v2', array['name', 'system_code', 'legacy_invoice_number', 'status', 'legacy_status']::text[]),
      ('tasks', 'idx_tasks_global_search_text_v2', array['name', 'title', 'system_code', 'status', 'description']::text[]),
      ('projects', 'idx_projects_global_search_text_v2', array['name', 'title', 'system_code', 'status', 'description', 'customer_name']::text[])
    ) items(table_name, index_name, columns)
  loop
    if to_regclass(format('public.%I', idx.table_name)) is null then
      continue;
    end if;

    select string_agg(format('coalesce(%I::text, '''')', key), ' || '' '' || ' order by ord)
      into v_index_expression
    from unnest(idx.columns) with ordinality item(key, ord)
    where exists (
      select 1
      from pg_catalog.pg_attribute a
      where a.attrelid = to_regclass(format('public.%I', idx.table_name))
        and a.attname = key
        and a.attnum > 0
        and not a.attisdropped
    );

    if v_index_expression is not null then
      v_index_sql := format(
