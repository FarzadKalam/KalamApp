...
type Comment = { id: string; media_id: string; author_username?: string | null; author_name?: string | null; author_profile_photo_url?: string | null; content_text: string; like_count: number; commented_at?: string | null };
  ...
  const load = useCallback(async (silent = false) => {
    ...
    try {
      const [mediaResult, commentsResult] = await Promise.all([
        ...
        supabase.from('instagram_comments').select('id,media_id,author_username,author_name,author_profile_photo_url,content_text,like_count,commented_at').order('commented_at', { ascending: false }).limit(80),
      ]);
  ...
  return <div className="grid min-h-0 flex-1 grid-cols-1 lg:grid-cols-[240px_minmax(0,1fr)]">
    ...
    <section className="min-h-0 overflow-y-auto bg-[linear-gradient(180deg,rgba(248,250,252,0.94),rgba(241,245,249,0.82))] p-4 dark:bg-none dark:bg-[#101113]">
      ...
      {loading ? <div className="flex justify-center py-16"><Spin /></div> : <>
        ...
        {selectedMedia ? <div className="mt-6 rounded-2xl border border-slate-200/70 bg-white/85 p-3 shadow-sm dark:border-white/[0.08] dark:bg-white/[0.045]"><div className="mb-3 flex flex-wrap items-center justify-between gap-2"><div className="flex items-center gap-2"><CommentOutlined className="text-[rgb(var(--brand-700-rgb))] dark:text-[rgb(var(--brand-200-rgb))]" /><span className="font-medium">کامنت‌های پست انتخاب‌شده</span></div><Button size="small" disabled={!selectedMedia.permalink} onClick={openMediaWorkflow}>گردش‌کار این پست</Button></div><List locale={{ emptyText: 'کامنتی برای این پست دریافت نشده است.' }} dataSource={visibleComments} renderItem={(comment) => { const status = commentStatuses[comment.id]; return <List.Item className="block"><div className="flex gap-2"><Avatar src={comment.author_profile_photo_url}>{(comment.author_name || comment.author_username || 'ا')[0]}</Avatar><div className="min-w-0 flex-1"><div className="text-sm font-medium">{comment.author_name || comment.author_username || 'کاربر اینستاگرام'}</div><div className="mt-1 whitespace-pre-wrap text-sm">{comment.content_text}</div><div className="mt-1 flex flex-wrap items-center gap-2 text-xs text-slate-400"><span>{dateText(comment.commented_at)}</span>{status ? <Tag color={status.color}>{status.label}</Tag> : null}</div><div className="mt-2 flex gap-2"><Input size="small" value={reply} onChange={(event) => setReply(event.target.value)} placeholder="پاسخ به کامنت..." onPressEnter={() => void replyToComment(comment)} /><Button size="small" type="primary" icon={<SendOutlined />} loading={sending} onClick={() => void replyToComment(comment)}>پاسخ</Button></div></div></div></List.Item>; }} /></div> : null}
      </>}
    </section>
  </div>;
};
