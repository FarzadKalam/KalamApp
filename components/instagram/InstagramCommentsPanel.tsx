...
const InstagramCommentsPanel: React.FC<{ accounts: Account[]; activeAccountId: string; onAccountChange: (id: string) => void }> = ({ accounts, activeAccountId, onAccountChange }) => {
  ...
  return <div className="grid min-h-0 flex-1 grid-cols-1 lg:grid-cols-[240px_minmax(0,1fr)]">
    <aside className="min-h-0 overflow-y-auto border-l border-slate-200 p-3 dark:border-white/[0.08]"><div className="mb-3 text-sm font-semibold">پیج‌های متصل</div><button type="button" onClick={() => onAccountChange('all')} className={`mb-1 w-full rounded-lg px-2 py-2 text-right text-sm ${activeAccountId === 'all' ? 'bg-pink-50 text-pink-700 dark:bg-pink-500/10 dark:text-pink-200' : ''}`}>همه پیج‌ها</button>{accounts.map((account) => <button key={account.id} type="button" onClick={() => onAccountChange(account.id)} className={`mb-1 flex w-full items-center gap-2 rounded-lg px-2 py-2 text-right text-sm ${activeAccountId === account.id ? 'bg-pink-50 text-pink-700 dark:bg-pink-500/10 dark:text-pink-200' : ''}`}><Avatar size={28} src={account.profile_photo_url}>{account.username?.[0]}</Avatar><span className="truncate">@{account.username}</span></button>)}</aside>
    <section className="min-h-0 overflow-y-auto p-4"><div className="mb-4 flex items-center justify-between"><div className="font-semibold">پست‌ها و کامنت‌ها</div><Button size="small" icon={<ReloadOutlined />} loading={loading} onClick={() => void syncPosts()}>دریافت پست‌ها</Button></div>{loading ? <div className="flex justify-center py-16"><Spin /></div> : <><div className="mb-5"><div className="mb-2 text-sm font-medium">استوری‌ها</div><div className="flex gap-3 overflow-x-auto pb-2">{stories.length ? stories.map((story) => <button key={story.id} type="button" className={`shrink-0 text-center ${selectedMediaId === story.id ? 'text-pink-600' : ''}`} onClick={() => setSelectedMediaId(story.id)}><Avatar size={62} src={story.thumbnail_url || story.media_url} className="ring-2 ring-pink-500 ring-offset-2" /><div className="mt-1 w-16 truncate text-[11px]">استوری</div></button>) : <span className="text-xs text-slate-400">استوری دریافت نشده است.</span>}</div></div><div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-4">{posts.map((post) => <button type="button" key={post.id} className={`overflow-hidden rounded-xl border text-right ${selectedMediaId === post.id ? 'border-pink-500 ring-2 ring-pink-200' : 'border-slate-200 dark:border-white/10'}`} onClick={() => setSelectedMediaId(post.id)}>{post.thumbnail_url || post.media_url ? <Image preview={false} src={post.thumbnail_url || post.media_url} height={150} className="w-full object-cover" /> : <div className="flex h-36 items-center justify-center bg-slate-100 text-slate-400">بدون تصویر</div>}<div className="p-2 text-xs"><div className="line-clamp-2">{post.caption || 'بدون کپشن'}</div><div className="mt-1 text-slate-400">♥ {Number(post.metrics?.like_count || 0).toLocaleString('fa-IR')} · ◌ {Number(post.metrics?.comments_count || 0).toLocaleString('fa-IR')}</div></div></button>)}</div>{posts.length === 0 ? <Empty className="mt-6" description="هنوز پستی از BoxAPI دریافت نشده است." /> : null}{selectedMedia ? <div className="mt-6 rounded-xl border border-slate-200 p-3 dark:border-white/10"><div className="mb-3 flex flex-wrap items-center justify-between gap-2"><div className="flex items-center gap-2"><CommentOutlined /><span className="font-medium">کامنت‌های {selectedMedia.media_type === 'story' ? 'استوری' : 'رسانه'} انتخاب‌شده</span></div><Button size="small" disabled={!selectedMedia.permalink} onClick={openMediaWorkflow}>گردش‌کار این {selectedMedia.media_type === 'story' ? 'استوری' : 'پست'}</Button></div><List locale={{ emptyText: 'کامنتی برای این پست دریافت نشده است.' }} dataSource={visibleComments} renderItem={(comment) => <List.Item className="block"><div className="flex gap-2"><Avatar src={comment.author_profile_photo_url}>{(comment.author_name || comment.author_username || 'ا')[0]}</Avatar><div className="min-w-0 flex-1"><div className="text-sm font-medium">{comment.author_name || comment.author_username || 'کاربر اینستاگرام'}</div><div className="mt-1 whitespace-pre-wrap text-sm">{comment.content_text}</div><div className="mt-1 text-xs text-slate-400">{dateText(comment.commented_at)} · ♥ {Number(comment.like_count || 0).toLocaleString('fa-IR')}</div><div className="mt-2 flex gap-2"><Input size="small" value={reply} onChange={(event) => setReply(event.target.value)} placeholder="پاسخ به کامنت..." onPressEnter={() => void replyToComment(comment)} /><Button size="small" type="primary" icon={<SendOutlined />} loading={sending} onClick={() => void replyToComment(comment)}>پاسخ</Button></div></div></div></List.Item>} /></div> : null}</>}</section>
  </div>;
};
