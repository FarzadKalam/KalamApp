  ...
  return <div className="grid min-h-0 flex-1 grid-cols-1 lg:grid-cols-[240px_minmax(0,1fr)]">
    ...
    <section className="min-h-0 overflow-y-auto bg-[linear-gradient(180deg,rgba(248,250,252,0.94),rgba(241,245,249,0.82))] p-4 dark:bg-none dark:bg-[#101113]">
      ...
      {loading ? <div className="flex justify-center py-16"><Spin /></div> : <>
        ...
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-4">{posts.map((post) => { const previewUrl = mediaPreviewUrl(post); const isVideo = isVideoUrl(post.media_url); return <button type="button" key={post.id} className={`overflow-hidden rounded-2xl border text-right transition ${selectedMediaId === post.id ? 'border-[rgba(var(--brand-500-rgb),0.55)] ring-2 ring-[rgba(var(--brand-500-rgb),0.18)] dark:border-[rgba(var(--brand-300-rgb),0.45)]' : 'border-slate-200 dark:border-white/10'}`} onClick={() => setSelectedMediaId(post.id)}>{previewUrl ? <Image preview={false} src={previewUrl} height={150} className="w-full object-cover" /> : isVideo ? <div className="relative h-36 overflow-hidden bg-slate-100 dark:bg-white/[0.05]"><video className="h-full w-full object-cover" src={post.media_url || undefined} muted playsInline preload="metadata" /><span className="absolute inset-0 flex items-center justify-center bg-black/10 text-2xl text-white drop-shadow"><PlayCircleOutlined /></span></div> : <div className="flex h-36 flex-col items-center justify-center gap-1 bg-slate-100 text-slate-400 dark:bg-white/[0.05]"><PlayCircleOutlined className="text-xl" /><span className="text-xs">تصویر پست در دسترس نیست</span></div>}<div className="bg-white/85 p-2 text-xs dark:bg-white/[0.055]"><div className="line-clamp-2">{post.caption || 'بدون کپشن'}</div></div></button>; })}</div>
