...
type SourceRecord = { id: string; moduleId: 'products' | 'billboards'; title: string; image_url?: string | null; description?: string | null; sell_price?: number | null; price?: number | null; unit_name?: string | null };
...
const InstagramShowcasesSettingsModal: React.FC<{ open: boolean; onClose: () => void; accounts: Account[]; defaultAccountId?: string | null }> = ({ open, onClose, accounts, defaultAccountId }) => {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [data, products, billboards, catalogRows, priceRows] = await Promise.all([
        ...
        supabase.from('products').select('id,name,image_url,description,sell_price,unit_name').limit(100),
  ...
  const save = async () => {
    try {
      ...
      setSaving(true);
      const items = selectedRecords.map((record) => ({ source_module_id: record.moduleId, source_record_id: record.id, snapshot: { title: record.title, image_url: record.image_url || null, description: record.description || null, price: record.sell_price ?? null, unit_name: record.unit_name || null } }));
