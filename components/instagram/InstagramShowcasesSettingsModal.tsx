...
const InstagramShowcasesSettingsModal: React.FC<{ open: boolean; onClose: () => void; accounts: Account[]; defaultAccountId?: string | null }> = ({ open, onClose, accounts, defaultAccountId }) => {
  ...
  return <Modal open={open} onCancel={onClose} title="تنظیمات پیام‌های دکمه‌دار" width={1100} footer={null} destroyOnHidden>
    <div className="grid grid-cols-1 gap-4 lg:grid-cols-[280px_minmax(0,1fr)]">
      <div className="rounded-xl border border-slate-200 p-3 dark:border-white/10"><Button type="primary" block icon={<PlusOutlined />} loading={loading} onClick={openCreate}>پیام دکمه‌دار جدید</Button><div className="mt-3 space-y-2">{showcases.length ? showcases.map((showcase) => <div key={showcase.id} className={`rounded-lg border p-2 ${editing?.id === showcase.id ? 'border-pink-400 bg-pink-50 dark:bg-pink-500/10' : 'border-slate-200 dark:border-white/10'}`}><button type="button" className="w-full text-right" onClick={() => openEdit(showcase)}><div className="font-medium">{showcase.name}</div><div className="mt-1 text-xs text-slate-500">{showcase.source_kind === 'manual' ? `${showcase.items?.length || 0} کالا یا خدمت` : showcase.source_kind === 'price_list' ? 'لیست قیمت' : 'کاتالوگ آنلاین'}</div></button><Popconfirm title="پیام دکمه‌دار حذف شود؟" okText="حذف" cancelText="انصراف" onConfirm={() => void remove(showcase.id)}><Button className="mt-1" danger type="text" size="small" icon={<DeleteOutlined />}>حذف</Button></Popconfirm></div>) : <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="پیام دکمه‌داری ساخته نشده است." />}</div></div>
      <div className="min-w-0"><Form form={form} layout="vertical" initialValues={{ sourceKind: 'manual', buttons: [], isActive: true }}><div className="grid grid-cols-1 gap-x-3 md:grid-cols-2"><Form.Item name="name" label="نام پیام دکمه‌دار" rules={[{ required: true, message: 'نام پیام دکمه‌دار را وارد کنید.' }]}><Input placeholder="مثلا معرفی محصولات پرفروش" /></Form.Item><Form.Item name="accountId" label="پیج پیش‌فرض"><Select allowClear options={accounts.map((account) => ({ value: account.id, label: `@${account.username}` }))} placeholder="برای همه پیج‌ها" /></Form.Item></div><Form.Item name="description" label="متن پیام"><Input.TextArea rows={2} /></Form.Item><div className="grid grid-cols-1 gap-x-3 md:grid-cols-2"><Form.Item name="sourceKind" label="منبع اطلاعات پیام"><Select options={[{ value: 'manual', label: 'انتخاب کالا و خدمات' }, { value: 'price_list', label: 'لیست قیمت' }, { value: 'online_catalog', label: 'کاتالوگ آنلاین' }]} /></Form.Item><Form.Item name="isActive" label="وضعیت" valuePropName="checked"><Switch checkedChildren="فعال" unCheckedChildren="غیرفعال" /></Form.Item></div>{sourceKind === 'manual' ? <Form.Item name="recordIds" label="کالاها و خدمات پیام"><Select mode="multiple" showSearch optionFilterProp="label" options={records.map((record) => ({ value: `${record.moduleId}:${record.id}`, label: `${record.moduleId === 'billboards' ? 'تبلیغات محیطی' : 'کالا/خدمت'}: ${record.title}` }))} placeholder="کالاها و خدمات را انتخاب کنید" maxTagCount="responsive" /></Form.Item> : <Form.Item name="sourceId" label={sourceKind === 'price_list' ? 'لیست قیمت' : 'کاتالوگ آنلاین'} rules={[{ required: true, message: 'منبع پیام را انتخاب کنید.' }]}><Select options={(sourceKind === 'price_list' ? priceLists.map((item) => ({ value: item.id, label: item.name })) : catalogs.map((item) => ({ value: item.id, label: item.title }))) } /></Form.Item>}<div className="rounded-xl border border-slate-200 p-3 dark:border-white/10"><div className="mb-2 font-medium">دکمه‌های پیام <span className="text-xs text-slate-500">(حداکثر ۳)</span></div><Form.List name="buttons">{(fields, { add, remove: removeButton }) => <div className="space-y-2">{fields.map((field) => <div key={field.key} className="grid grid-cols-1 gap-2 md:grid-cols-[1fr_1.2fr_1.4fr_auto]"><Form.Item name={[field.name, 'title']} rules={[{ required: true, message: 'عنوان لازم است.' }]}><Input placeholder="عنوان" /></Form.Item><Form.Item name={[field.name, 'action_type']} rules={[{ required: true, message: 'اقدام لازم است.' }]}><Select options={BUTTON_ACTIONS} placeholder="عملکرد" /></Form.Item><Form.Item name={[field.name, 'payloadText']}><Input placeholder="لینک، متن یا کلید رویداد" /></Form.Item><Button danger type="text" onClick={() => removeButton(field.name)}>حذف</Button></div>)}{fields.length < 3 ? <Button type="dashed" icon={<PlusOutlined />} onClick={() => add({ action_type: 'postback' })}>افزودن دکمه</Button> : null}</div>}</Form.List></div>{sourceKind === 'manual' && selectedRecords.length ? <div className="mt-4"><div className="mb-2 font-medium">پیش‌نمایش اطلاعات پیام</div><div className="flex gap-3 overflow-x-auto pb-2">{selectedRecords.map((record) => <Card key={`${record.moduleId}:${record.id}`} className="w-48 shrink-0" cover={record.image_url ? <Image preview={false} height={120} src={record.image_url} className="object-cover" /> : undefined}><Card.Meta title={record.title} description={<><div>{record.price ? `${Number(record.price).toLocaleString('fa-IR')} ریال` : 'قیمت ثبت نشده'}</div>{record.main_unit ? <Tag>{record.main_unit}</Tag> : null}</>} /></Card>)}</div></div> : null}<Space className="mt-5"><Button type="primary" loading={saving} onClick={() => void save()}>ذخیره پیام دکمه‌دار</Button><Button onClick={openCreate}>انصراف از ویرایش</Button></Space></Form></div>
    </div>
  </Modal>;
};
