...
const SimpleLineChart: React.FC<{ items: SimpleLineChartItem[]; valueFormatter?: (value: number) => React.ReactNode }> = ({ items, valueFormatter }) => {
  ...
  return <div className="space-y-3 overflow-x-auto"><svg viewBox={`0 0 ${width} ${height}`} className="min-w-[620px] w-full" role="img" aria-label="نمودار خطی گزارش">
    ...
    {points.map((point, index) => <g key={`${point.label}-${index}`}><circle cx={point.x} cy={point.y} r="5" fill={point.tone === 'decrease' || point.value < 0 ? '#dc2626' : '#16a34a'} /><text x={point.x} y={height - 7} textAnchor="middle" fontSize="11" fill="#64748b">{point.label}</text></g>)}
  </svg><div className="flex flex-wrap gap-3 text-xs text-gray-600 dark:text-gray-300">{safeItems.map((item) => <span key={item.label}><b className={item.tone === 'decrease' || item.value < 0 ? 'text-red-600' : 'text-green-600'}>{item.label}</b>: {formatter(item.value)}</span>)}</div></div>;
};
