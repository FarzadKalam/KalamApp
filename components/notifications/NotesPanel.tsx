...
// ---------------------------------------------------------------------------
// Conversation sidebar (desktop) — owns its own virtualizer so scrolling the
// conversation list re-renders only this component, never the message thread.
// ---------------------------------------------------------------------------
type ConversationSidebarProps = {
  allConvItems: ConvListItem[];
  selectedNoteUserId: string | null;
  systemMessagesUserId: string;
  myNoteStats: any;
  effectiveSystemNoteStats: any;
  systemConversationAvatar: any;
  systemAvatarSrc: string | null;
  UnifiedConversationAvatar: any;
  buildNoteConversationAvatarModel: any;
  noteUserSearch: string;
  setNoteUserSearch: (value: string) => void;
...
const ConversationSidebar: React.FC<ConversationSidebarProps> = React.memo(({
  ...
  const renderConvItem = (listItem: ConvListItem) => {
    ...
    if (listItem.type === 'system') {
      return (
        <button
          type="button"
          onClick={() => onSelectConversation(systemMessagesUserId)}
          className={`w-full rounded-xl px-3 py-2 text-right transition-colors ${selectedNoteUserId === systemMessagesUserId ? activeConversationClass : inactiveConversationClass}`}
        >
    ...
    <div dir="rtl" className="order-last w-[208px] border-l border-slate-200/55 bg-white/72 dark:border-white/[0.07] dark:bg-white/[0.025]">
      <div className="px-4 py-3 border-b border-slate-200/45 bg-white/55 dark:border-white/[0.07] dark:bg-white/[0.025]">
        <div className="flex items-center justify-between gap-2">
          ...
          <Button size="small" shape="circle" icon={<PlusOutlined />} onClick={onOpenGroupEditor} />
        </div>
...
// ---------------------------------------------------------------------------
// Conversation rail (mobile) — same isolation as the desktop sidebar.
// ---------------------------------------------------------------------------
type ConversationRailProps = {
  ...
  noteUserSearch: string;
  setNoteUserSearch: (value: string) => void;
  mobileNoteSearchOpen: boolean;
  setMobileNoteSearchOpen: (open: boolean) => void;
  onSelectConversation: (id: string | null) => void;
  onOpenGroupEditor: () => void;
};

const ConversationRail: React.FC<ConversationRailProps> = React.memo(({
  allConvItems,
  selectedNoteUserId,
  systemMessagesUserId,
  myNoteStats,
  ...
  const renderRailItem = (listItem: ConvListItem) => {
    if (listItem.type === 'my-notes') {
      return (
        <button
          type="button"
    ...
    if (listItem.type === 'system') {
      return (
        <button
          type="button"
          onClick={() => onSelectConversation(systemMessagesUserId)}
          className={`flex w-full flex-col items-center gap-1 rounded-2xl px-1 py-1.5 transition-colors ${selectedNoteUserId === systemMessagesUserId ? activeRailClass : inactiveRailClass}`}
        ...
        >
          <div className="relative">
            <Badge count={effectiveSystemNoteStats.unreadCount > 0 ? toPersianNumber(String(effectiveSystemNoteStats.unreadCount)) : 0} size="small" offset={[-2, 2]} color={NOTIFICATION_UNREAD_BADGE_COLOR}>
              <UnifiedConversationAvatar size={38} src={systemConversationAvatar.src} className={`${selectedNoteUserId === systemMessagesUserId ? 'ring-2 ring-[rgba(var(--brand-500-rgb),0.42)] ring-offset-2 ring-offset-white dark:ring-[rgba(var(--brand-300-rgb),0.55)] dark:ring-offset-[#151113]' : ''} ${systemConversationAvatar.className || ''}`.trim()} fallback={systemConversationAvatar.fallback} />
            </Badge>
          </div>
    ...
    <div dir="rtl" className="w-[54px] shrink-0 overflow-hidden border-l border-slate-200/45 bg-white/60 dark:border-white/[0.07] dark:bg-white/[0.025]">
      <div className="flex h-full flex-col items-center gap-0 overflow-hidden">
        ...
        <div className="sticky top-0 z-10 flex w-full flex-col items-center gap-0.5 px-1 pt-1.5 pb-1 bg-white/60 dark:bg-white/[0.025]">
          ...
          <Button type="default" shape="circle" size="small" icon={<PlusOutlined />} className="shadow-sm" onClick={onOpenGroupEditor} />
        </div>
        {/* Virtualized rail */}
...
// ---------------------------------------------------------------------------
// Single message row — memoized so virtualizer scroll frames (which re-render
// the timeline) bail out for unchanged rows: same note + same ctx ⇒ no work.
// ---------------------------------------------------------------------------
type NoteMessageRowProps = {
  note: any;
  replyTarget: any | null;
  ctx: Record<string, any>;
};

const NoteMessageRow: React.FC<NoteMessageRowProps> = React.memo(({ note, replyTarget, ctx }) => {
  const {
    profileId,
    recordTitleMap,
    formatRecordLabel,
  ...
  const isMine = !isSystem && note.author_id && profileId && note.author_id === profileId;
  ...
  const replyParsedContent = useMemo(() => (replyTarget ? parseNoteContent(replyTarget.content) : null), [replyTarget]);
  const replyAuthorName = replyTarget
    ? (replyTarget.author_id && profileId && replyTarget.author_id === profileId ? 'شما' : (replyTarget.author_name || directoryUserMap[String(replyTarget.author_id || '')]?.display_name || authorNameMap[replyTarget.author_id] || 'کاربر سیستم'))
    : null;
  const parsedContent = useMemo(() => parseNoteContent(note.content), [note.content]);
  ...
  const likedByMe = Boolean(likeReceiptMapFromBox(note.metadata)[String(profileId || '').trim()]);
  const noteAvatar = resolveNoteBubbleAvatar(note, Boolean(isMine), isSystem);
  const createdAtLabel = useMemo(() => safeJalaliFormat(note.created_at, 'YYYY/MM/DD HH:mm'), [note.created_at]);
          ...
          Modal.confirm({
            ...
            onOk: async () => {
              ...
              const { error } = await supabase
                ...
                .delete()
                .eq('id', noteId)
                .eq('author_id', String(profileId || '').trim());
...
NoteMessageRow.displayName = 'NoteMessageRow';
...
const NotesPanel: React.FC<NotesPanelProps> = ({ layout, context }) => {
  ...
  const allConvItems = useMemo<ConvListItem[]>(() => [
  ...
  const handleSelectConversation = useCallback((id: string | null) => {
    ...
    setSelectedNoteUserId(id);
  }, [setMobileNoteSearchOpen, setSelectedNoteUserId]);
  ...
  }, [setEditingGroup, setGroupMemberDrafts, setGroupNameDraft, setGroupModalOpen]);
  ...
  // Stable row context: rows only re-render when something they show changes,
  // never because the timeline virtualizer produced a new scroll frame.
  const noteRowCtx = useMemo(() => ({
  ...
  const renderNoteRow = useCallback((note: any) => {
    if (!note) return null;
    const replyTarget: any = note.reply_to ? noteMap.get(note.reply_to) : null;
    return <NoteMessageRow note={note} replyTarget={replyTarget || null} ctx={noteRowCtx} />;
  }, [noteMap, noteRowCtx]);

  const getNoteKey = useCallback((note: any, index: number) => String(note?.id ?? index), []);
  ...
  return (
    <div dir="ltr" className="flex min-w-0 flex-1 min-h-0 overflow-hidden bg-[rgba(var(--brand-50-rgb),0.16)] dark:bg-[#151113]">
      ...
      {withUserSidebar ? (
        <ConversationSidebar
          allConvItems={allConvItems}
          selectedNoteUserId={selectedNoteUserId}
          systemMessagesUserId={SYSTEM_MESSAGES_USER_ID}
          myNoteStats={myNoteStats}
          effectiveSystemNoteStats={effectiveSystemNoteStats}
          systemConversationAvatar={systemConversationAvatar}
          systemAvatarSrc={systemAvatarSrc}
          UnifiedConversationAvatar={UnifiedConversationAvatar}
          buildNoteConversationAvatarModel={buildNoteConversationAvatarModel}
          noteUserSearch={noteUserSearch}
          setNoteUserSearch={setNoteUserSearch}
          onSelectConversation={handleSelectConversation}
          onOpenGroupEditor={handleOpenGroupEditor}
        />
      ) : null}
      ...
      {withMobileUserRail ? (
        <ConversationRail
          ...
          noteUserSearch={noteUserSearch}
          setNoteUserSearch={setNoteUserSearch}
          mobileNoteSearchOpen={mobileNoteSearchOpen}
          setMobileNoteSearchOpen={setMobileNoteSearchOpen}
          onSelectConversation={handleSelectConversation}
          onOpenGroupEditor={handleOpenGroupEditor}
        />
      ) : null}
