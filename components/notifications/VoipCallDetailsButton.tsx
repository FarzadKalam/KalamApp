...
const VoipCallDetailsButton: React.FC<VoipCallDetailsButtonProps> = ({ call, compact = false }) => {
  ...
  const rows = [
    ...
    ['داخلی اپراتور', displayValue(call?.extension)],
    ['داخلی مقصد', displayValue(call?.target_extension || (direction === 'incoming' ? call?.extension : null))],
    ['نام مقصد داخلی', displayValue(call?.target_endpoint_name)],
