      ...
      <div className="invoice-summary-card mt-8 bg-leather-600 rounded-2xl p-6 text-white shadow-xl shadow-leather-500/30 flex flex-col md:flex-row md:flex-wrap md:justify-between md:items-center gap-6">
        ...
        <div className="flex flex-col md:flex-row gap-6 md:gap-8 text-center w-full md:w-auto">
          <div className="w-full md:w-auto">
            ...
            <div className="text-2xl font-black font-['Peyda'] persian-number">
              {formatValue(data.total)} <span className="text-sm font-normal opacity-80 font-['Peyda']">{currencyLabel}</span>
            </div>
          </div>
          {data.received !== undefined && (
            <>
              ...
              <div className="w-full md:w-auto">
                ...
                <div className="text-2xl font-black font-['Peyda'] persian-number text-green-100">
                  {formatValue(data.received)} <span className="text-sm font-normal opacity-80 font-['Peyda']">{currencyLabel}</span>
                </div>
              </div>
            </>
          )}
          {data.remaining !== undefined && (
            <>
              ...
              <div className="w-full md:w-auto">
                ...
                <div className="text-2xl font-black font-['Peyda'] persian-number text-orange-100">
                  {formatValue(data.remaining)} <span className="text-sm font-normal opacity-80 font-['Peyda']">{currencyLabel}</span>
                </div>
              </div>
            </>
          )}
  ...
  if (data.total > 0) {
    return (
      <div className="invoice-summary-card mt-8 bg-leather-600 rounded-2xl p-4 flex items-center justify-between shadow-lg shadow-leather-500/20">
        ...
        <div className="text-white font-black text-2xl font-['Peyda'] persian-number">
          {formatValue(data.total)} <span className="text-sm font-normal opacity-80 font-['Peyda']">{currencyLabel}</span>
        </div>
      </div>
    );
