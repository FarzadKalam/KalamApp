...
const FUNCTION_BUILD = 'taxpayer-system-2026-04-15-01';
...
const buildPacket = (args: { packetType: string; data?: any; uid?: string | null; retry?: boolean; encryptionKeyId?: string; symmetricKey?: string; iv?: string; fiscalId?: string; dataSignature?: string; signatureKeyId?: string | null; }) => ({ uid: args.uid ?? null, packetType: args.packetType, retry: args.retry === true, data: args.data ?? null, encryptionKeyId: args.encryptionKeyId || '', symmetricKey: args.symmetricKey || '', iv: args.iv || '', fiscalId: args.fiscalId || '', dataSignature: args.dataSignature || '', signatureKeyId: args.signatureKeyId ?? null });
...
const getToken = async (settings: any, privateKey: string) => { const tokenRes = await invokeSignedSync(settings, privateKey, 'GET_TOKEN', { username: settings.fiscal_id }, undefined, { fiscalId: settings.fiscal_id }); const token = String(tokenRes?.result?.data?.token || tokenRes?.data?.token || tokenRes?.result?.token || tokenRes?.token || tokenRes?.access_token || '').trim(); if (!token) throw new Error('GET_TOKEN succeeded but no access token was returned.'); return { token }; };
...
const invoicePayload = (args: any) => { const { invoice, customer, products, company, settings, txid, serial, settlement } = args; const invDate = String(invoice.invoice_date || '').slice(0,10); const items = Array.isArray(invoice.invoiceItems) ? invoice.invoiceItems : []; if (!items.length) throw new Error('Invoice has no rows to send.'); const currency = String(company?.currency_code || 'IRT'); const settlementCode = setm(settlement); const buyerType = String(customer?.person_type || 'real') === 'legal' ? 2 : 1; const buyerId = buyerType === 2 ? String(customer?.national_id || '').trim() : String(customer?.national_code || '').trim(); if (!customer?.id || !buyerId) throw new Error('Customer identity data is incomplete.'); let tprdis=0, tdis=0, tadis=0, tvam=0, tbill=0; const body = items.map((item: any, i: number) => { const product = products[String(item?.product_id || '')] || {}; const sstid = String(product?.product_identifier || '').trim(); if (!sstid) throw new Error(`Product/service identifier is missing for row ${i + 1}.`); const qty = Number(item?.quantity || 0); if (!(qty > 0)) throw new Error(`Quantity is invalid for row ${i + 1}.`); const a = rowAmounts(item); const base = rial(a.base,currency), dis = rial(a.dis,currency), after = rial(a.after,currency), vat = rial(a.vat,currency), total = rial(a.total,currency); const measureUnitCode = normalizeOptionalNumber(item?.measure_unit_code ?? item?.mu ?? product?.measure_unit_code ?? product?.main_unit); tprdis += base; tdis += dis; tadis += after; tvam += vat; tbill += total; return { sstid, sstt: String(item?.description || product?.name || 'Product/Service'), mu: measureUnitCode, am: qty, fee: rial(item?.unit_price || 0,currency), cfee: null, cut: null, exr: null, ssu: null, prdis: base, dis, adis: after, vra: Number(a.vatRate || 0), vam: vat, odt: null, odr: null, odam: null, olt: null, olr: null, olam: null, consfee: null, cop: null, vop: vat, tsstam: total, bsrn: null }; }); const received = rial(invoice.total_received_amount || 0, currency); const cap = settlementCode === 1 ? tbill : settlementCode === 2 ? null : Math.min(Math.max(received, 0), tbill); const insp = settlementCode === 2 ? tbill : settlementCode === 1 ? null : Math.max(tbill - (cap || 0), 0); const indatim = new Date(`${invDate}T00:00:00Z`).getTime(); return { packetType: 'INVOICE.V01', data: { header: { taxid: txid, inno: BigInt(serial).toString(16).toUpperCase().padStart(10,'0'), indatim, indati2m: indatim, inty: Number(invoice.taxpayer_invoice_type || 1), inp: Number(invoice.taxpayer_invoice_pattern || 1), ins: Number(invoice.taxpayer_invoice_subject || 1), tins: settings.seller_economic_code, tob: buyerType, bid: buyerId, tinb: String(customer?.economic_code || '').trim() || undefined, bpc: String(customer?.postal_code || '') || undefined, setm: settlementCode, tprdis, tdis, tadis, tvam, todam: 0, tbill, cap, insp, tvop: tvam, tax17: null }, body, payments: [] } }; };
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === 'test_connection' || action === 'sync_server_information') { const { settings, privateKey } = await requireSettings(urlBase,key,orgId); const info = await invokeGetServerInformation(settings); await upsertSettings(urlBase,key,{ id: settings.id, org_id: orgId, server_information: info || {}, updated_by: user.id, updated_at: new Date().toISOString() }); if (action === 'test_connection') await getToken({ ...settings, server_information: info || {} }, privateKey); return json(200, { success: true, message: action === 'test_connection' ? 'Server information received and GET_TOKEN succeeded.' : 'Server information received.', server_information: info }); }
    if (action === 'send_invoice') {
      const invoiceId = String(body?.invoice_id || '').trim();
      if (!invoiceId) return json(400, { success: false, message: 'invoice_id is required.' });

      const { settings, company, privateKey } = await requireSettings(urlBase,key,orgId);
      let currentSettings = settings;
      if (!serverKey(currentSettings.server_information || {})) {
        const info = await invokeGetServerInformation(settings);
      ...
      const debugPayload = { build: FUNCTION_BUILD, taxpayer_settings: { fiscal_id: currentSettings.fiscal_id, base_url: currentSettings.base_url } };

      let sub = first(await insert(urlBase,key,'taxpayer_invoice_submissions',[{
        org_id: orgId,
        invoice_id: invoiceId,
        fiscal_id: currentSettings.fiscal_id,
        internal_serial: Number(serial),
        taxid: txid,
        status: 'sending',
        invoice_type: String(bundle.invoice.taxpayer_invoice_type || '1'),
        invoice_pattern: String(bundle.invoice.taxpayer_invoice_pattern || '1'),
        invoice_subject: String(bundle.invoice.taxpayer_invoice_subject || '1'),
        settlement_method: settlement,
        request_payload: { _kalam_debug: { ...debugPayload, stage: 'preflight' } },
        created_by: user.id,
      }]));
      ...
      try {
        let payload;
        try {
          payload = invoicePayload({ ...bundle, company, settings: currentSettings, txid, serial, settlement });
        } catch (e: any) {
          const message = `build-invoice-payload: ${String(e?.message || e)}`;
          const failureDebug = { ...debugPayload, stage: 'build-invoice-payload' };
          await patch(urlBase,key,'taxpayer_invoice_submissions',sub.id,{ status: 'failed', error_message: message, response_payload: { _kalam_debug: failureDebug }, updated_at: new Date().toISOString() });
          throw new Error(message);
        }

        const enqueueMeta = requestMeta();
        let builtPacket;
        try {
          builtPacket = await buildEncryptedInvoicePacket(currentSettings, privateKey, payload, enqueueMeta.requestTraceId);
        } catch (e: any) {
          const message = `build-packet: ${String(e?.message || e)}`;
          const failureDebug = e?.kalamDebug ? { ...debugPayload, stage: 'build-packet', packet_debug: e.kalamDebug } : { ...debugPayload, stage: 'build-packet' };
          await patch(urlBase,key,'taxpayer_invoice_submissions',sub.id,{ status: 'failed', error_message: message, request_payload: { ...payload, _kalam_debug: failureDebug }, response_payload: { _kalam_debug: failureDebug }, updated_at: new Date().toISOString() });
          throw new Error(message);
        }

        const packet = builtPacket.packet;
        const packetDebug = { ...debugPayload, stage: 'build-packet', packet_debug: builtPacket.debug };
        ...
        let token;
        try {
          ({ token } = await getToken(currentSettings, privateKey));
        } catch (e: any) {
          const message = `GET_TOKEN: ${String(e?.message || e)}`;
          const failureDebug = { ...packetDebug, stage: 'GET_TOKEN' };
          await patch(urlBase,key,'taxpayer_invoice_submissions',sub.id,{ status: 'failed', error_message: message, response_payload: { _kalam_debug: failureDebug }, updated_at: new Date().toISOString() });
          throw new Error(message);
        }
        ...
        let enqueueResult;
        try {
          enqueueResult = await invokeAsyncEnqueue(currentSettings, privateKey, packet, token, enqueueMeta);
        } catch (e: any) {
          const failureDebug = e?.kalamDebug ? { ...packetDebug, stage: 'normal-enqueue', enqueue_debug: e.kalamDebug } : { ...packetDebug, stage: 'normal-enqueue' };
          const message = `normal-enqueue: ${String(e?.message || e)}`;
          await patch(urlBase,key,'taxpayer_invoice_submissions',sub.id,{ status: 'failed', error_message: message, response_payload: { _kalam_debug: failureDebug }, updated_at: new Date().toISOString() });
          throw new Error(message);
        }

        const res = enqueueResult.response;
        const debugResult = { ...packetDebug, stage: 'normal-enqueue', enqueue_debug: enqueueResult.debug };
