-- =====================================================
-- KalamApp - Phase 106
-- Web forms required text autofill for public intake
-- =====================================================
...
begin
  ...
  if not exists (
    ...
    where t.table_schema = 'public' and t.table_name = v_form.target_module_id
  ) then
  ...
  if exists (
    select 1 from information_schema.columns c
    where c.table_schema = 'public' and c.table_name = v_form.target_module_id
      and c.column_name = 'title' and c.is_nullable = 'NO'
  ...
  if exists (
    select 1 from information_schema.columns c
    where c.table_schema = 'public' and c.table_name = v_form.target_module_id
      and c.column_name = 'name' and c.is_nullable = 'NO'
  ...
  if exists (
    select 1 from information_schema.columns c
    where c.table_schema = 'public' and c.table_name = v_form.target_module_id
      and c.column_name = 'full_name' and c.is_nullable = 'NO'
  ...
  if exists (
    select 1 from information_schema.columns c
    where c.table_schema = 'public' and c.table_name = v_form.target_module_id
      and c.column_name = 'subject' and c.is_nullable = 'NO'
  ...
  values (
    v_org_id, v_form.id, v_form.target_module_id, v_target_record_id, 'submitted',
    v_submission, v_record_payload, v_meta
  )
...
exception
  when others then
    if v_form.id is not null then
      ...
      values (
        v_org_id, v_form.id, v_form.target_module_id, 'failed',
