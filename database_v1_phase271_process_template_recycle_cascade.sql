...
create index if not exists idx_recycle_bin_process_template_stage_parent
  on public.recycle_bin_records ((snapshot->>'template_id'))
  where source_table = 'process_template_stages';

create index if not exists idx_recycle_bin_workflow_process_template_parent
  on public.recycle_bin_records ((snapshot->>'process_template_id'))
