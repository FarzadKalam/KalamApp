-- =====================================================
-- KalamApp - Phase 506: Existing member can own one new demo organization
-- Date: 2026-09-15
...
begin
  ...
  select public.normalize_demo_mobile(u.phone)
  into current_auth_mobile
  from auth.users u
  where u.id = current_user_id;
  if current_auth_mobile is distinct from normalized_mobile then
    raise exception 'demo_phone_session_mismatch';
  end if;

  select * into current_profile
  from public.profiles
  where id = current_user_id;
  if current_profile.id is null or coalesce(current_profile.is_active, true) = false then
    raise exception 'demo_member_profile_invalid';
  end if;
  if not exists (
    select 1
    ...
    raise exception 'demo_member_access_missing';
  end if;
  if exists (
    select 1
    from public.user_organization_memberships m
    ...
    raise exception 'demo_owner_already_exists';
  end if;
  ...
  insert into public.saas_onboarding_requests (
    ...
    current_user_id, trim(p_full_name), normalized_mobile, nullif(trim(coalesce(p_business_name, '')), ''),
    nullif(trim(coalesce(p_employee_count_band, '')), ''), nullif(trim(coalesce(p_discovery_source, '')), ''), normalized_slug,
    normalized_owner_email, normalized_industry, normalized_palette_key, 'started', true
  ...
  insert into public.user_organization_memberships (
    user_id, org_id, role_id, software_role, is_owner, is_active
  ) values (
    current_user_id, target_org_id, admin_role_id, 'admin', true, true
  );

  target_redirect_host := normalized_slug || '.tazesystem.ir';
  insert into public.saas_org_settings (
    org_id, slug, status, plan_code, trial_ends_at, is_demo, is_readonly,
    requested_subdomain, resolved_host, provisioning_source, dns_status,
    primary_contact_mobile, created_by, updated_by
  ) values (
    target_org_id, normalized_slug, 'demo', target_plan.code,
    now() + make_interval(days => target_trial_days), true, false,
    normalized_slug, target_redirect_host, 'self_service', 'active',
    normalized_mobile, current_user_id, current_user_id
  );

  insert into public.saas_demo_issuance (
    mobile, auth_user_id, org_id, request_id, issued_by, issuance_mode
  ) values (
    normalized_mobile, current_user_id, target_org_id, request_row.id,
    current_user_id, 'self_service'
  );

  begin
    insert into public.company_settings (
      org_id, company_name, company_full_name, trade_name, brand_palette_key,
      ceo_name, mobile, email, updated_by
    ) values (
      target_org_id, display_business_name, display_business_name, display_business_name,
      normalized_palette_key, trim(p_full_name), regexp_replace(normalized_mobile, '^\+98', '0'),
      normalized_owner_email, current_user_id
    );
  exception when others then
    optional_warnings := optional_warnings || jsonb_build_array(jsonb_build_object('step', 'company_settings', 'message', SQLERRM));
  end;
  ...
  exception when others then
    optional_warnings := optional_warnings || jsonb_build_array(jsonb_build_object('step', 'integration_settings', 'message', SQLERRM));
  end;
  ...
  begin
    insert into public.marketing_leads (
      ...
      trim(p_full_name), normalized_mobile,
  ...
  exception when others then
    optional_warnings := optional_warnings || jsonb_build_array(jsonb_build_object('step', 'marketing_leads', 'message', SQLERRM));
  end;
  ...
  set org_id = target_org_id,
      ...
      notes = case when jsonb_array_length(optional_warnings) = 0 then notes
        else concat_ws(E'\n', nullif(notes, ''), 'optional_warnings=' || optional_warnings::text) end,
      updated_at = now()
...
exception
  ...
  if request_row.id is not null then
    update public.saas_onboarding_requests
    set status = case when coalesce(status, '') in ('', 'draft', 'started') then 'failed' else status end,
        provision_attempts = provision_attempts + 1,
...
revoke all on function public.provision_self_service_demo_for_existing_member(text, text, text, text, text, text, text, text, text) from public;
revoke all on function public.provision_self_service_demo_for_existing_member(text, text, text, text, text, text, text, text, text) from anon;
grant execute on function public.provision_self_service_demo_for_existing_member(text, text, text, text, text, text, text, text, text) to authenticated;
