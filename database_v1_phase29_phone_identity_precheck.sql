-- =====================================================
-- KalamApp - Phase 29 Phone Identity Precheck
-- Date: 2026-03-22
-- Type: Additive / non-breaking migration
-- Goal: distinguish users that exist in auth.users from users that also have a phone identity in auth.identities
-- =====================================================
...
begin
  ...
  where i.provider = 'phone'
    and (
      i.identity_data ->> 'phone' = normalized_phone
      or i.user_id in (
        select u.id
        from auth.users u
        where u.phone = normalized_phone
