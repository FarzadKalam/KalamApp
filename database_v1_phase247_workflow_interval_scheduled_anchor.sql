-- Phase 247: Keep server-side interval workflows anchored to scheduled time
-- Goal: daily/monthly/hourly interval workflows should calculate the next run
--       from the configured schedule, not from the actual runner claim time.
...
declare
  ...
  v_i integer;
begin
  if v_unit not in ('hour', 'day', 'month') then
    v_unit := 'day';
  end if;
    ...
    if p_interval_first_run_at is not null then
      ...
      if v_unit = 'hour' then
        ...
        if p_interval_day_of_month is not null then
          v_month_days := extract(day from (date_trunc('month', v_now_local) + interval '1 month' - interval '1 day'))::integer;
  ...
  for v_i in 1..10000 loop
    if v_unit = 'hour' then
      ...
      if p_interval_day_of_month is not null then
        v_month_days := extract(day from (date_trunc('month', v_candidate_local) + interval '1 month' - interval '1 day'))::integer;
        v_target_day := least(greatest(p_interval_day_of_month, 1), v_month_days);
    ...
    and (
      ...
      or (
        ...
        and (
          ...
          or (w.interval_allowed_from_hour > w.interval_allowed_to_hour
            and (v_hour >= w.interval_allowed_from_hour or v_hour <= w.interval_allowed_to_hour))
        )
      )
      or (w.interval_allowed_from_hour is not null and w.interval_allowed_to_hour is null
          and v_hour >= w.interval_allowed_from_hour)
      or (w.interval_allowed_from_hour is null and w.interval_allowed_to_hour is not null
