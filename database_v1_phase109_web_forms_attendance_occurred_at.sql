-- =====================================================
-- KalamApp - Phase 109
-- Web forms: attendance_logs occurred_at backfill
-- =====================================================
...
begin
        ...
        select coalesce(
          jsonb_agg(
            jsonb_build_object(
              ...
              'target_field_key', v_field.target_field_key
            )
          ),
          '[]'::jsonb
        )
  ...
  if v_existing_record_id is not null and v_duplicate_strategy = 'skip' then
    ...
    where c.table_schema = 'public'
      ...
      and exists (
        select 1
        from jsonb_each(v_record_payload) payload
      ...
      if v_target_has_org_id then
        execute format(
          'update public.%1$I as t
           set %2$s
           from jsonb_populate_record(null::public.%1$I, $3) as src
           where t.id = $2
             and t.org_id = $1
        ...
        using v_org_id, v_existing_record_id, v_record_payload;
      else
        execute format(
          'update public.%1$I as t
           set %2$s
    ...
    if nullif(v_target_record->>'id', '') is not null then
      ...
      v_target_record_id_text := v_target_record->>'id';
    end if;

    v_submission_record_payload := v_insert_payload;
  end if;

  if jsonb_array_length(v_record_files) > 0 and v_target_record_id_text <> '' and exists (
    select 1
    from information_schema.tables t
    where t.table_schema = 'public'
    ...
    select
      ...
      nullif(trim(coalesce(item->>'url', '')), ''),
      nullif(trim(coalesce(item->>'name', '')), ''),
      nullif(trim(coalesce(item->>'mime_type', '')), ''),
      coalesce(nullif(trim(coalesce(item->>'file_type', '')), ''), 'file'),
      auth.uid()
    from jsonb_array_elements(v_record_files) item
    where nullif(trim(coalesce(item->>'url', '')), '') is not null;
  end if;
  ...
  values (
    ...
    v_submission,
    case
      when coalesce(v_submission_record_payload, '{}'::jsonb) = '{}'::jsonb then v_record_payload
      else v_submission_record_payload
    end,
    v_meta
  )
